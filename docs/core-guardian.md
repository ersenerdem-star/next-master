...
8. Production shipping must bootstrap local Netlify CLI link state before deploy verification. `npm run netlify:ensure-link`, `npm run predeploy:verify`, and `npm run ship` keep `.netlify/state.json` bound to `ersen-quote-desk` locally, then `npm run deploy:status` still checks the live Git-connected production URL.
...
- `npm run netlify:ensure-link` writes the local `.netlify/state.json` binding for `ersen-quote-desk` when the CLI state is missing or stale.
