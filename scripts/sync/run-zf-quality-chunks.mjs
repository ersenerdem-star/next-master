...
import { isCatalogPlaceholderDescription, normalizeCatalogEan } from "../shared/catalog/catalog-standardization.mjs";
...
const supabaseUrl = resolveSyncEnvValue("SUPABASE_URL", { projectRoot }).trim().replace(/\/+$/, "");
const serviceRoleKey = resolveSyncEnvValue("SUPABASE_SERVICE_ROLE_KEY", { projectRoot }).trim();
...
for (const brandName of brands) {
      ...
      JSON.stringify(
        {
          ...
          descriptionRows: result.description_rows,
          eanRows: result.ean_rows,
  ...
  aggregate.push({
    ...
    aggregateDescriptionRows: sum(chunkResults.map((entry) => entry.result.description_rows || 0)),
    aggregateEanRows: sum(chunkResults.map((entry) => entry.result.ean_rows || 0)),
...
async function fetchAllCatalogRows(organizationId, brandId) {
  ...
  for (let offset = 0; ; offset += pageSize) {
    const page = await fetchJson(
      `${supabaseUrl}/rest/v1/catalog_products?select=product_code,normalized_code,description,ean,oem_no,vehicle,hs_code,origin,weight_kg,image_url,lifecycle_status,lifecycle_note&organization_id=eq.${organizationId}&brand_id=eq.${brandId}&limit=${pageSize}&offset=${offset}`,
    );
...
function shouldProcessRow(row) {
  ...
  if (!normalizeCatalogEan(row.ean || "")) return true;
