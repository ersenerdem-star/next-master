...
async function main() {
  ...
  if (importMode && rows.length) {
    ...
    const payload = rows.map((row) => ({
      ...
      ...(supportsEanColumn ? { ean: emptyToNull(row.ean_number) } : {}),
...
function extractHengstProduct(html, filePath, brand) {
  ...
  const description = pickCatalogDescription(
    [extractDescription(html, productCode), extractMetaDescription(html), extractProductName(html)],
    productCode,
  );
...
function extractDescription(html, productCode) {
  ...
  const cleaned = decodeHtml(snippet.replace(/<[^>]+>/g, " "))
    ...
    .trim();
  if (cleaned && !isCatalogPlaceholderDescription(cleaned, productCode)) return cleaned;
...
function mergeRows(existing, incoming) {
  return {
    ...
    ean_number: preferValue(existing.ean_number, incoming.ean_number),
...
async function detectCatalogEanColumn() {
  try {
    await getJson("/rest/v1/catalog_products?select=ean&limit=1");
    return true;
  } catch (error) {
