...
import { isCatalogPlaceholderDescription, normalizeCatalogDisplayCode, normalizeCatalogEan } from "../shared/catalog/catalog-standardization.mjs";
...
if (!supabaseUrl || !serviceRoleKey) {
  throw new Error("SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required in env, .sync-secrets.local, CLI args, or Netlify env");
}
...
const resolveRelatedCodes = !args.has("skip-related-codes");
...
async function main() {
  ...
  for (const target of targets) {
    ...
    for (const row of existingRows) {
      if (missingOnly) {
        if (shouldProcessRow(row, supportsEanColumn)) {
      ...
      if (refreshExisting || shouldProcessRow(row, supportsEanColumn)) {
        workMap.set(row.normalized_code, {
          target,
          existing: row,
          searchItem: discoveredSearchMap.get(row.normalized_code) || null,
    ...
    const extraSearchItems = resolveRelatedCodes
      ? await resolveExtraCodes({
          target,
          extraCodes,
          existingByCode,
          processedCodes,
  ...
  const summary = {
    ...
    description_rows: matched.filter((row) => !isCatalogPlaceholderDescription(row.description, row.product_code)).length,
    ean_rows: matched.filter((row) => normalizeCatalogEan(row.ean || "")).length,
    ...
    resolve_related_codes: resolveRelatedCodes,
...
function normalizeDetailPayload(target, payload) {
  ...
  const ean = extractEan(details, specEntries);
...
function extractEan(details, specEntries) {
  const direct = normalizeCatalogEan(String(details?.mainStage?.ean?.value || details?.mainStage?.ean || ""));
  if (direct) return direct;
  return normalizeCatalogEan(extractSpecValue(specEntries, [/^ean$/i, /gtin/i, /barcode/i, /ean\s*code/i]));
}
...
function shouldProcessRow(row, supportsEanColumn) {
...
function hasCatalogDelta(existing, next, supportsEanColumn) {
  return (
