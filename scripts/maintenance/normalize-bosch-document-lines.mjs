...
const tableConfigs = [
  {
    table: "sales_orders",
    labelField: "sales_order_no",
    lineFields: ["requestedCode", "resolvedCode"],
  },
  {
    table: "invoices",
    labelField: "sales_order_no",
    lineFields: ["product_code", "old_code"],
  },
  {
...
const summaryPath = path.join(outputDir, `bosch-document-line-normalization-summary-${timestamp}.json`);
const csvPath = path.join(outputDir, `bosch-document-line-normalization-changes-${timestamp}.csv`);
  ...
  if (apply && updates.length) {
    for (let index = 0; index < updates.length; index += batchSize) {
      ...
      for (const row of batch) {
        ...
        if (!response.ok) {
          throw new Error(`${config.table} Bosch document line update failed for ${row.id}: ${response.status} ${text}`);
        }
      }
...
function normalizeLines(lines, fields) {
  ...
  const nextLines = lines.map((line) => {
    if (normalizeBrandKey(line?.brand || "") !== "bosch") return line;
    let lineChanged = false;
    const nextLine = { ...line };
    for (const field of fields) {
      const current = String(nextLine[field] || "").trim();
      if (!current) continue;
      const next = normalizeBoschCode(current);
