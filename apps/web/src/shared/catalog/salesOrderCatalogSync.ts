...
import { shouldUseCPriceForCustomer, type CustomerPricingMode } from "./pricingPolicy";
...
async function refreshLinePricesFromCatalog(
  ...
  try {
    ...
    const cPriceMap =
      shouldUseCPriceForCustomer(options.customerType, options.customerPricingMode || "standard")
    ...
    const cSellPrice = shouldUseCPriceForCustomer(options.customerType, options.customerPricingMode || "standard")
