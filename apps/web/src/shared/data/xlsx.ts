...
function buildSheetXml(rows: Array<Array<string | number | null | undefined>>, numericColumns: number[]) {
  ...
  const body = rows
    .map((row, rowIndex) => {
      ...
      while (endIndex >= 0) {
        const value = row[endIndex];
        if (value != null && String(value).trim() !== "") break;
        endIndex -= 1;
      }
      const visibleRow = endIndex >= 0 ? row.slice(0, endIndex + 1) : [];
      const cells = visibleRow.map((cell, colIndex) => xlsxCell(cell, rowIndex, colIndex, numericSet)).join("");
