...
export function getStatementPeriodBounds(periodType: StatementPeriodType, anchorDate: string) {
  const anchor = new Date(anchorDate || "");
...
type BuildAccountStatementHtmlInput = {
  title: string;
...
export function buildAccountStatementHtml(input: BuildAccountStatementHtmlInput, options?: { autoPrint?: boolean }) {
  ...
  <html>
    <head>
      ...
      <style>
        body { font-family: Arial, sans-serif; color:#1f2937; margin:28px; font-size:12px; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
...
export function openAccountStatementPrintWindow(input: BuildAccountStatementHtmlInput, options?: { autoPrint?: boolean }) {
  const popup = window.open("", "_blank", "noopener,noreferrer");
  if (!popup) {
    throw new Error("Popup blocked while opening statement view.");
  }

  popup.document.write(buildAccountStatementHtml(input, options));
