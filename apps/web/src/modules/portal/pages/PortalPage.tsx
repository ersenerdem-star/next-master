...
import { buildAccountStatementHtml, openAccountStatementPrintWindow } from "../../../shared/accountStatementPrint";
import { downloadPdfFromHtml } from "../../../shared/pdfDownload";
import { translateAppText, type AppLanguage, type AppLanguagePreference } from "../../../shared/i18n";
...
const PORTAL_SEARCH_CACHE_TTL_MS = 5 * 60 * 1000;
...
function summarizeBrands(brands: string[], maxVisible = 4) {
...
type PortalPageProps = {
  ...
  languagePreferenceOptions?: ReadonlyArray<{ value: AppLanguagePreference; label: string }>;
  languageStatusLabel?: string;
  onLanguagePreferenceChange?: (value: AppLanguagePreference) => void;
};
...
function buildPortalFallbackPreparedLine(
  row: { code: string; brand: string; qty: number },
  reason: string,
): PortalPreparedLine {
  const requestedCode = String(row.code || "").trim();
  const brand = String(row.brand || "").trim();
  return {
    lineId: `portal-fallback-${requestedCode || "unknown"}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    requestedCode,
    ...
    c_sell_price: null,
    price_date: "",
    notes: String(reason || "Live pricing unavailable. Please retry smaller batches."),
    found: false,
    codeChanged: false,
    codeChangeWarning: "",
...
function getPortalSearchCacheKey(email: string, query: string, brand: string) {
  return `${getPortalCacheKey(email)}:search:${normalizePartCode(query || "__empty__")}::${String(brand || "").trim().toLowerCase() || "all"}`;
}
...
function readPortalSearchCache(email: string, query: string, brand: string) {
  if (typeof window === "undefined" || !email || (!query.trim() && !brand.trim())) return null as PortalSearchCacheEntry | null;
  try {
    const raw = window.localStorage.getItem(getPortalSearchCacheKey(email, query, brand));
    if (!raw) return null;
    const parsed = JSON.parse(raw) as PortalSearchCacheEntry;
    if (!parsed?.updatedAt || Date.now() - Number(parsed.updatedAt) > PORTAL_SEARCH_CACHE_TTL_MS) return null;
    return parsed;
  } catch {
    return null;
  }
...
function writePortalSearchCache(email: string, query: string, brand: string, value: PortalSearchCacheEntry | null) {
  if (typeof window === "undefined" || !email || (!query.trim() && !brand.trim())) return;
  const key = getPortalSearchCacheKey(email, query, brand);
  if (!value) {
    window.localStorage.removeItem(key);
    return;
  }
...
export function PortalPage({
  ...
  const t = (key: Parameters<typeof translateAppText>[1], variables?: Record<string, string | number>) =>
    translateAppText(language, key, variables);
  ...
  const [portalPriceListScopeMode, setPortalPriceListScopeMode] = useState<"all" | "brand">("brand");
  ...
  useEffect(() => {
    if (snapshot) {
      setLoginBranding({
        ...
        portalLabel: snapshot.invite.party_type === "customer" ? t("portal.customer_portal") : t("portal.vendor_portal"),
  ...
  useEffect(() => {
    ...
    setStatus(isOnline ? t("portal.status.cached_workspace_loaded") : t("portal.status.offline_workspace_loaded"));
    setError("");
      ...
      .then(({ snapshot: next }) => {
        ...
        setStatus((current) =>
          current && current.toLowerCase().includes("offline")
            ? t("portal.status.data_refreshed")
            : current || t("portal.status.data_refreshed"),
        );
      })
      .catch((caught) => {
        if (cancelled) return;
  ...
  }, [credentials.email, isOnline, language, portalResetToken, snapshot]);
  ...
  const accountColumns = useMemo(
    () => [
      { key: "type", header: "Document", render: (row: PortalSnapshot["accountRows"][number]) => row.document_type },
      {
        key: "no",
        header: "No",
        render: (row: PortalSnapshot["accountRows"][number]) => (
          <span className="portal-document-no" title={row.document_no}>
            {row.document_no}
          </span>
        ),
      },
    ...
    () => [
      {
        ...
        render: (row: PortalSnapshot["salesOrders"][number]) => (
          <div>
            <strong className="portal-document-no" title={row.sales_order_no || row.id}>
              {row.sales_order_no || row.id}
            </strong>
    ...
    () => [
      {
        key: "no",
        header: "Invoice",
        render: (row: PortalSnapshot["invoices"][number]) => <span className="portal-document-no" title={row.id}>{row.id}</span>,
      },
      {
        key: "sales",
        header: "Sales Order",
        render: (row: PortalSnapshot["invoices"][number]) => (
          <span className="portal-document-no" title={row.sales_order_no || "-"}>
            {row.sales_order_no || "-"}
          </span>
        ),
      },
    ...
    () => [
      {
        ...
        render: (row: PortalSnapshot["purchaseOrders"][number]) => <span className="portal-document-no" title={row.id}>{row.id}</span>,
      },
      {
        key: "sales",
        header: "Sales Order",
        render: (row: PortalSnapshot["purchaseOrders"][number]) => (
          <span className="portal-document-no" title={row.sales_order_no || "-"}>
            {row.sales_order_no || "-"}
          </span>
        ),
    ...
    () => [
      {
        key: "no",
        header: "Bill",
        render: (row: PortalSnapshot["bills"][number]) => <span className="portal-document-no" title={row.id}>{row.id}</span>,
      },
      {
        key: "po",
        header: "Purchase Order",
        render: (row: PortalSnapshot["bills"][number]) => (
          <span className="portal-document-no" title={row.purchase_order_no || "-"}>
    ...
    () => [
      {
        key: "no",
        header: "Payment",
        render: (row: PortalSnapshot["paymentsReceived"][number] | PortalSnapshot["paymentsMade"][number]) => (
          <span className="portal-document-no" title={row.id}>
            {row.id}
          </span>
        ),
      },
    ...
    () => {
      const columns = [
        ...
        },
        {
          ...
          header: "Code",
          render: (row: PortalCatalogSearchItem) => (
            <span className="portal-result-code" title={row.code || "-"}>
              {row.code || "-"}
            </span>
          ),
        },
        ...
        },
        {
          ...
          render: (row: PortalCatalogSearchItem) => (
            <span className={`portal-result-price price-tag price-tag--compact${row.sell_price == null ? " price-tag--muted" : ""}`}>
      ...
      const columns = [
        {
          ...
          render: (row: PortalPreparedLine) => (
            <span className="portal-result-code" title={row.resolvedCode || row.requestedCode || "-"}>
              {row.resolvedCode || row.requestedCode || "-"}
            </span>
          ),
        },
  ...
  async function handleLogin() {
    try {
      ...
      setBrandFilter("");
      setPaymentStatusFilter("");
      setStatus(t("portal.status.session_active"));
      const nextCredentials = { email: credentials.email, password: "", sessionToken: "" };
      setCredentials(nextCredentials);
      writeStoredCredentials(nextCredentials);
  ...
  async function handleRefresh() {
    if (!isOnline) {
      setError("");
      setStatus(t("portal.status.connect_to_refresh"));
      return;
    }
    try {
      setLoading(true);
      ...
      setSnapshot(next);
      setSelection(null);
      setStatus(t("portal.status.data_refreshed"));
      const nextCredentials = { email: credentials.email, password: "", sessionToken: "" };
      setCredentials(nextCredentials);
      writeStoredCredentials(nextCredentials);
    } catch (caught) {
    ...
    return (
      <div className="portal-shell portal-shell--login">
        <div className="login-layout login-layout--portal">
          <DakarLoginShowcase
            theme="portal"
            brandLabel={loginBrandLabel}
            brandName={loginBrandName}
            logoDataUrl={loginBrandLogo}
            fallbackMonogram={loginBrandInitials}
          />
          <div className="login-card portal-login-card">
            {languagePreferenceOptions.length ? (
              <label className="field portal-language-switcher portal-language-switcher--login">
                <span className="field__label">{t("shell.language")}</span>
                <select
                  className="field__input"
                  value={languagePreference}
                  onChange={(event) => onLanguagePreferenceChange?.(event.target.value as AppLanguagePreference)}
                >
                ...
                {languageStatusLabel ? <span className="portal-language-switcher__hint">{languageStatusLabel}</span> : null}
              </label>
            ) : null}
            <div className="login-card__eyebrow">
              {portalResetToken
                ? t("portal.login.reset_eyebrow")
                : portalForgotMode
                  ? t("portal.login.recovery_eyebrow")
                  : t("portal.login.sign_in_eyebrow")}
            </div>
            <h1>{portalResetToken ? t("portal.login.create_password_title") : t("portal.login.portal_access_title")}</h1>
            <p>
              {portalResetToken
                ? t("portal.login.set_password_hint")
                : portalForgotMode
                  ? t("portal.login.enter_email_hint")
                  : t("portal.login.sign_in_hint")}
            </p>
            <form
            ...
            >
              <Input label={t("portal.login.email")} value={credentials.email} placeholder={t("portal.login.email_placeholder")} onChange={(value) => setCredentials((current) => ({ ...current, email: value }))} />
              {portalResetToken ? (
                <>
                  <Input
                    label={t("portal.login.new_password")}
                    type="password"
                    value={portalResetPassword}
                    placeholder={t("portal.login.new_password_placeholder")}
                    onChange={setPortalResetPassword}
                  />
                  <Input
                    label={t("portal.login.confirm_password")}
                    type="password"
                    value={portalResetConfirmPassword}
                    placeholder={t("portal.login.repeat_password_placeholder")}
                ...
                <Input
                  ...
                  placeholder={t("portal.login.password_placeholder")}
              ...
              <div className="inline-actions">
                <Button
                  type="submit"
                  busy={loading}
                  busyLabel={portalResetToken ? t("portal.login.saving") : portalForgotMode ? t("portal.login.sending") : t("portal.login.signing_in")}
                  onClick={() => {
                    if (portalResetToken) {
                      void handlePortalResetPassword();
                      return;
                ...
                >
                  {portalResetToken ? t("portal.login.save_new_password") : portalForgotMode ? t("portal.login.send_reset_link") : t("portal.login.sign_in")}
                </Button>
              </div>
              ...
              {!portalResetToken ? (
                ...
                >
                  {portalForgotMode ? t("portal.login.back_to_sign_in") : t("portal.login.forgot_password")}
                </button>
              ) : null}
            </form>
          </div>
  ...
  const portalPricingLabel =
    !activeSnapshot.pricingProfile
      ? t("portal.price.default_pricing")
      : portalCPriceMode === "prefer_c_when_available"
        ? t("portal.price.c_where_available")
        : portalBasePriceListType
          ? `${portalBasePriceListType} Price List`
          : t("portal.price.customer_account_pricing");
  ...
  const portalAvailableBrandSummary = summarizeBrands(activeSnapshot.availableBrands, 3);
  const portalAvailableBrandCount = activeSnapshot.availableBrands.length;
  const portalScopeLabel = orderSearchBrand ? orderSearchBrand : `${portalAvailableBrandCount.toLocaleString("en-US")} brands included`;
  const portalScopeCaption = orderSearchBrand ? "Scoped brand" : portalAvailableBrandSummary || t("portal.price.all_allowed_brands");
  const portalScopeTooltip = activeSnapshot.availableBrands.join(", ");
  ...
  const brandOptions = (() => {
    ...
    return [{ value: "", label: t("portal.price.all_brands") }, ...Array.from(brands).sort((a, b) => a.localeCompare(b)).map((brand) => ({ value: brand, label: brand }))];
  })();
  ...
  const portalBrandOptions = [{ value: "", label: t("portal.price.all_brands") }, ...activeSnapshot.availableBrands.map((brand) => ({ value: brand, label: brand }))];
  const portalBrandValueOptions = activeSnapshot.availableBrands.map((brand) => ({ value: brand, label: brand }));
  const portalPriceListScopeLabel = portalPriceListScopeMode === "all" ? t("portal.price.all_brands") : portalPriceListBrand || t("portal.price.brand_select");
  const portalPriceListScopeCaption =
    portalPriceListScopeMode === "all"
      ? portalAvailableBrandSummary || t("portal.price.all_allowed_brands")
      : portalPriceListBrand
        ? t("portal.price.single_brand_download")
        : t("portal.price.select_a_brand");
  const portalPriceListScopeTooltip =
    portalPriceListScopeMode === "all" ? portalAvailableBrandSummary || activeSnapshot.availableBrands.join(", ") : portalPriceListBrand;
  ...
  const portalOriginalNumberBrandSummary = summarizeBrands(portalOriginalNumberBrandMatches, 5);
  ...
  const portalQuickStats = [
    {
      label: portalCanOrder ? t("portal.quick.open_baskets") : t("portal.quick.orders"),
      value: portalCanOrder ? portalDraftOrders.length.toLocaleString("en-US") : portalOrderHistoryRows.length.toLocaleString("en-US"),
      note: portalCanOrder ? t("portal.quick.saved_work_waiting") : t("portal.quick.visible_order_records"),
    },
    {
      label: activeSnapshot.invite.party_type === "customer" ? t("portal.quick.invoices") : t("portal.quick.bills"),
      value: portalBillingRows.length.toLocaleString("en-US"),
      note: t("portal.quick.document_history"),
    },
    {
      label: t("portal.quick.balance"),
      value: formatMoney(activeSnapshot.accountSummary.openAmount, activeSnapshot.accountSummary.currency),
      note: t("portal.quick.open_account"),
    },
    {
      label: t("portal.quick.payments"),
      value: formatMoney(activeSnapshot.accountSummary.paymentAmount, activeSnapshot.accountSummary.currency),
      note: t("portal.quick.payment_movement"),
    },
  ];
  const portalSections: Array<{ key: PortalSection; label: string }> = [
    ...(portalCanOrder ? [{ key: "desk" as PortalSection, label: t("portal.section.part_search") }] : []),
    ...(portalCanOrder ? [{ key: "pricelist" as PortalSection, label: t("portal.section.download_price_list") }] : []),
    ...(activeSnapshot.invite.access.can_view_orders ? [{ key: "orders" as PortalSection, label: t("portal.section.orders") }] : []),
    ...(activeSnapshot.invite.access.can_view_invoices
      ? [{ key: "billing" as PortalSection, label: activeSnapshot.invite.party_type === "customer" ? t("portal.section.invoices") : t("portal.section.bills") }]
      : []),
    ...(activeSnapshot.invite.access.can_view_account || activeSnapshot.invite.access.can_view_payments ? [{ key: "statement" as PortalSection, label: t("portal.section.statement") }] : []),
    { key: "account", label: t("portal.nav.account") },
  ];
  const portalNavGroups: Array<{
  ...
  }> = [
    {
      key: "search" as const,
      code: "01",
      title: t("portal.nav.search"),
      caption: t("portal.nav.parts"),
      items: portalSections.filter((section) => section.key === "desk"),
    },
    {
      key: "pricing" as const,
      code: "02",
      title: t("portal.nav.price_lists"),
      caption: t("portal.nav.downloads"),
    ...
    },
    {
      ...
      caption: activeSnapshot.invite.party_type === "customer" ? t("portal.nav.orders_invoices") : t("portal.nav.po_bills"),
    ...
    },
    {
      ...
      code: "04",
      title: t("portal.nav.finance"),
      caption: t("portal.nav.statement_payments"),
      items: portalSections.filter((section) => section.key === "statement"),
    },
    {
      key: "account" as const,
      code: "05",
      title: t("portal.nav.account"),
      caption: t("portal.nav.profile"),
  ...
  const activeSectionHelpText =
    activeSection === "desk"
      ? t("portal.help.part_search")
      : activeSection === "pricelist"
        ? t("portal.help.price_list")
        : activeSection === "orders"
          ? t("portal.help.orders")
        : activeSection === "billing"
          ? t("portal.help.billing")
          : activeSection === "statement"
            ? t("portal.help.statement")
            : t("portal.help.account");
  ...
  async function handlePortalCatalogSearch() {
    const trimmedQuery = orderSearch.trim();
    const cachedSearch = trimmedQuery || orderSearchBrand ? readPortalSearchCache(credentials.email, trimmedQuery, orderSearchBrand) : null;
    if (!isOnline) {
      setError("");
      if (cachedSearch?.items?.length) {
        setCatalogResults(cachedSearch.items);
        setPortalRecommendations(cachedSearch.recommendations || []);
        if (cachedSearch.items[0]) {
          setSelectedCatalogCode(cachedSearch.items[0].code);
          setPortalPreview({ kind: "catalog", item: cachedSearch.items[0] });
        }
        setStatus("Offline mode active. Showing cached search results. Reconnect to refresh search.");
      } else if (catalogResults.length) {
        if (catalogResults[0]) {
          setSelectedCatalogCode(catalogResults[0].code);
          setPortalPreview({ kind: "catalog", item: catalogResults[0] });
        }
        setStatus("Offline mode active. Showing last in-memory search results.");
      } else {
    ...
    if (cachedSearch?.items?.length) {
      ...
      setStatus("Cached portal search loaded.");
      setPortalOrderStatus(
        `${cachedSearch.items.length.toLocaleString("en-US")} cached result(s) loaded.${cachedSearch.recommendations.length ? ` ${cachedSearch.recommendations.length.toLocaleString("en-US")} cached recommendation(s) restored.` : ""}`,
      );
      setError("");
      return;
    }
    try {
      ...
      writePortalSearchCache(credentials.email, trimmedQuery, orderSearchBrand, {
        query: trimmedQuery,
      ...
      if (cachedSearch?.items?.length) {
        ...
        setPortalRecommendations(cachedSearch.recommendations || []);
        setSelectedCatalogCode(cachedSearch.items[0]?.code || "");
        setPortalPreview(cachedSearch.items[0] ? { kind: "catalog", item: cachedSearch.items[0] } : null);
        setStatus("Live search failed. Cached portal results restored.");
      } else {
        setCatalogResults([]);
        setPortalRecommendations([]);
        setSelectedCatalogCode("");
    ...
    try {
      ...
      for (const chunk of chunks) {
        try {
          ...
          const chunkMessage = caught instanceof Error ? caught.message : "Portal order pricing failed";
          failedChunkMessage = chunkMessage;
          for (const row of chunk) {
            try {
              const prepared = await preparePortalOrderLinesApi(credentials, [row]);
              preparedLines = mergePortalPreparedLines(preparedLines, prepared.lines);
              setPortalDraftLines((current) => mergePortalPreparedLines(current, prepared.lines));
              latestPricingProfile = prepared.pricingProfile || latestPricingProfile;
              processed += 1;
            } catch (singleCaught) {
              const singleMessage = singleCaught instanceof Error ? singleCaught.message : chunkMessage;
              preparedLines = mergePortalPreparedLines(preparedLines, [buildPortalFallbackPreparedLine(row, singleMessage)]);
              setPortalDraftLines((current) => mergePortalPreparedLines(current, [buildPortalFallbackPreparedLine(row, singleMessage)]));
              processed += 1;
            }
          }
          continue;
  ...
  async function handleDownloadPortalPriceList() {
    const selectedPriceListBrand = portalPriceListScopeMode === "all" ? "" : portalPriceListBrand;
    if (portalPriceListScopeMode === "brand" && !selectedPriceListBrand) {
    ...
    try {
      ...
      setPortalOverlay({
        ...
        message: portalPriceListScopeMode === "all"
          ? "Building an all-brands price list for this customer account."
          : `Building ${selectedPriceListBrand} price list for this customer account.`,
      });
      const result = await downloadPortalPriceList(credentials, selectedPriceListBrand);
      if (!result.rows.length) {
        throw new Error("No catalog rows were returned for this portal price list. Download was not created.");
      }
      const scopeLabel = portalPriceListScopeMode === "all" ? "All Brands" : selectedPriceListBrand;
      const rows: Array<Array<string | number | null>> = [
        ["Brand", "Part_No", "Description", "Price_List_Type", `Price_${result.currency}`, "Currency", "Price_Date", "Lifecycle", "Lifecycle_Note"],
        ...result.rows.map((row) => [
          row.brand,
          row.product_code,
          row.description || "",
          ...
          result.currency,
          row.price_date || "",
          row.lifecycle_status === "discontinued" ? "Discontinued" : "Active",
          row.lifecycle_note || "",
        ]),
      ];
      const blob = buildXlsxBlob(`${scopeLabel} Price List`, rows, [4]);
      const fileSuffix =
        result.pricingMode === "prefer_c_when_available" && result.priceListType !== "C"
          ? "c-where-available"
          : result.priceListType;
      downloadBlob(`${sanitizeFileName(`portal-price-list-${portalPriceListScopeMode === "all" ? "all-brands" : selectedPriceListBrand}-${fileSuffix}`)}.xlsx`, blob);
      setStatus(
        portalPriceListScopeMode === "all"
          ? `All brands price list downloaded.`
          : result.pricingMode === "prefer_c_when_available" && result.priceListType !== "C"
            ? `${selectedPriceListBrand} price list downloaded using C where available and ${result.priceListType} fallback.`
            : `${selectedPriceListBrand} ${result.priceListType} price list downloaded.`,
      );
    } catch (caught) {
  ...
  function handleStatementPrint() {
    ...
    openAccountStatementPrintWindow(statementInput);
  }

  function handleStatementDownloadPdf() {
    const company = activeSnapshot.companyProfile
      ? {
          id: activeSnapshot.companyProfile.id || "portal-company",
          companyName: activeSnapshot.companyProfile.company_name || "Next Master",
    ...
    void downloadPdfFromHtml({
      html: buildAccountStatementHtml(statementInput),
      filename: `${sanitizeFileName(`${activeSnapshot.invite.party_name}-account-statement`)}.pdf`,
    }).catch((caught) => {
      setError(caught instanceof Error ? caught.message : "Statement PDF download failed.");
    });
  }
  ...
  function openPortalDocumentPrint(kind?: PortalSelection["kind"], id?: string) {
    ...
    const html = buildBusinessDocumentHtml({
      ...
      showOrderNoColumn: documentToPrint.kind !== "sales-order",
    });
    try {
      openBusinessDocumentPreview(html);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Popup blocked while opening PDF view.");
    }
  }
  ...
  function handlePortalDownloadPdf() {
    const documentToPrint = selectedDocument;
    if (!documentToPrint) return;
    const docNo =
      documentToPrint.kind === "sales-order"
        ? documentToPrint.row.sales_order_no || documentToPrint.row.id
    ...
    void downloadPdfFromHtml({
      html,
      filename: `${sanitizeFileName(docNo || documentToPrint.kind)}.pdf`,
    }).catch((caught) => {
      setError(caught instanceof Error ? caught.message : "Document PDF download failed.");
    });
  }
    ...
    <SectionCard
      ...
      actions={
        <div className="inline-actions">
          ...
          <Button variant="secondary" onClick={handlePortalDownloadPdf}>
            Download PDF
          </Button>
      ...
      <div className="portal-header">
        <div className="portal-brand">
          ...
          <div>
            ...
            <p>
              {activeSnapshot.invite.party_type === "customer" ? t("portal.customer_portal") : t("portal.vendor_portal")} for {activeSnapshot.invite.party_name}
            </p>
            {portalSellerDetails ? <div className="portal-brand__meta">{portalSellerDetails}</div> : null}
          </div>
        </div>
        <div className="inline-actions portal-header__actions">
          {languagePreferenceOptions.length ? (
            <label className="field portal-language-switcher">
              <span className="field__label">{t("shell.language")}</span>
              <select
                className="field__input"
                value={languagePreference}
          ...
          <Button variant="secondary" busy={loading} busyLabel={t("portal.refreshing")} onClick={() => void handleRefresh()}>
            {t("portal.refresh")}
          </Button>
      ...
      <div className="portal-layout">
        <aside className="portal-sidebar">
          <div className="brand-panel portal-sidebar__panel">
            <div className="brand-panel__eyebrow">{t("portal.navigation")}</div>
            <div className="brand">{activeSnapshot.invite.party_type === "customer" ? t("portal.customer") : t("portal.vendor")}</div>
        ...
        <div className="portal-content">
          ...
          {activeSection !== "desk" ? (
            <div className="portal-inline-note portal-inline-note--soft">
              <span>{t("portal.current_view")}</span>
            ...
            <SectionCard
              ...
              actions={
                <div className="portal-statement-actions">
                  ...
                  <Button variant="secondary" onClick={handleStatementDownloadPdf}>
                    Download PDF
                  </Button>
                </div>
              }
          ...
          {activeSnapshot.invite.access.can_view_account ? (
            <SectionCard title="Account Statement">
              <DataTable
                ...
                className="data-table--portal-documents"
                wrapClassName="table-wrap--records"
                emptyText="No statement rows available."
              />
            </SectionCard>
          ) : null}
          ...
          {activeSnapshot.invite.party_type === "vendor" && activeSnapshot.invite.access.can_view_invoices ? (
            <SectionCard title="Vendor Credits">
              <DataTable
                ...
                columns={vendorCreditColumns}
                className="data-table--portal-documents"
                wrapClassName="table-wrap--records"
                emptyText="No vendor credits available."
              />
            </SectionCard>
          ) : null}
        </div>
        ...
        <div className="portal-section-stack">
          <SectionCard title="Download Price List">
            <div className="portal-price-list-grid">
              <div className="portal-price-list-grid__brand">
                <div className="portal-price-list-grid__scope">
                  <Select
                    label="Scope"
                    value={portalPriceListScopeMode}
                    options={[
                      { value: "all", label: "All Brands" },
                      { value: "brand", label: "Brand Select" },
                    ]}
                    onChange={(value) => {
                      setPortalPriceListScopeMode(value === "all" ? "all" : "brand");
                    }}
                  />
                  {portalPriceListScopeMode === "brand" ? (
                    <Select label="Brand" value={portalPriceListBrand} options={portalBrandValueOptions} onChange={setPortalPriceListBrand} />
                  ) : (
                    <div className="portal-inline-note portal-inline-note--soft portal-inline-note--tight portal-inline-note--all">
                      <span>All Brands</span>
                      <strong>{portalAvailableBrandCount.toLocaleString("en-US")} allowed brands will export in one file.</strong>
                    </div>
                  )}
                </div>
              </div>
              <div className="portal-price-list-grid__stats">
                <div className="portal-filter-stat portal-filter-stat--compact">
                  <span>Customer Pricing</span>
                  <strong>{portalPricingLabel}</strong>
                  <small>{portalPricingRuleDescription}</small>
                </div>
                <div className="portal-filter-stat portal-filter-stat--compact portal-filter-stat--scope">
                  <span>Brand Scope</span>
                  <strong title={portalPriceListScopeTooltip}>{portalPriceListScopeLabel}</strong>
                  <small>{portalPriceListScopeCaption}</small>
                </div>
              </div>
              <div className="portal-price-list-grid__actions">
                <Button variant="secondary" busy={downloadingPortalPriceList} busyLabel="Preparing..." onClick={() => void handleDownloadPortalPriceList()}>
                  Download Excel
                </Button>
              </div>
            </div>
            <div className="portal-inline-note portal-inline-note--soft portal-inline-note--tight">
              <span>Pricing Rule</span>
              <strong>{portalPricingRuleDescription}</strong>
            </div>
            <div className="portal-inline-note portal-inline-note--soft portal-inline-note--tight">
              <span>Export Columns</span>
              <strong>Brand, part no, description, type, price, currency, date, lifecycle.</strong>
            </div>
          </SectionCard>
        </div>
          ) : null}
        ...
        <div className="portal-section-stack">
          <SectionCard title="Article Search" className="search-focus-card search-focus-card--portal">
            <div className="portal-order-builder">
              <form
                className="portal-search-dock"
                onSubmit={(event) => {
                  event.preventDefault();
                  void handlePortalCatalogSearch();
                }}
              >
                <div className="portal-search-dock__header">
                  <div>
                    <span className="portal-search-dock__eyebrow">Article Workbench</span>
                    <h3>Direct Article Search</h3>
                    <p className="portal-search-dock__summary">
                      Search part no, OEM reference, or description. Exact hits stay in the main result block; related alternatives are separated below.
                    </p>
                  </div>
                  <div className="portal-search-dock__hint">
                    <span>Brand Scope</span>
                    <strong title={portalScopeTooltip}>{portalScopeLabel}</strong>
                    <small>{portalScopeCaption}</small>
                  </div>
                </div>

                <div className="portal-search-dock__rail">
                  <div className="portal-search-dock__search">
                    <Input label="Article / OEM Search" value={orderSearch} placeholder="Enter article number, OEM, or description" onChange={setOrderSearch} />
                  </div>
                  <Select label="Brand Scope" value={orderSearchBrand} options={portalBrandOptions} onChange={setOrderSearchBrand} />
                  <Button type="submit" variant="secondary" busy={searchingCatalog} busyLabel="Searching..." onClick={() => void handlePortalCatalogSearch()}>
                    Search
                  </Button>
                </div>

                <div className="portal-filter-grid portal-filter-grid--desk">
                  <Select
                    label="Working Basket"
                    value={portalOrderId}
                ...
                <div className="portal-search-dock__metrics">
                  <div className="portal-search-dock__metric">
                    <span>Exact Results</span>
                    <strong>{catalogResults.length.toLocaleString("en-US")}</strong>
                  </div>
                  <div className="portal-search-dock__metric">
                    <span>Related Alternatives</span>
                    <strong>{portalRecommendationCards.length.toLocaleString("en-US")}</strong>
                  </div>
                  <div className="portal-search-dock__metric">
                    <span>Basket Lines</span>
                    <strong>{portalDraftLines.length.toLocaleString("en-US")}</strong>
                  </div>
                  <div className="portal-search-dock__metric portal-search-dock__metric--status">
                    <span>Pricing State</span>
                    <strong>
                      {portalDraftHasMissingPrices
                        ? "Pricing review"
                        : portalDraftDiscontinuedCount > 0
                          ? "Lifecycle review"
                          : "Ready to confirm"}
                  ...
                  {portalOriginalNumberBrandMatches.length ? (
                    <div className="portal-search-dock__metric portal-search-dock__metric--wide">
                      <span>Original No Match</span>
                      <strong>{portalOriginalNumberBrandSummary}</strong>
                    </div>
                  ) : null}
                </div>
              </form>
              ...
              <div className="portal-workbench">
                <div className="portal-workbench__tables">
                  <SectionCard title={`Search Items (${catalogResults.length.toLocaleString("en-US")})`} className="search-results-focus-card">
                          ...
                          >
                            <div className="portal-search-card__top">
                              ...
                              <div className="portal-search-card__meta">
                                ...
                                <div className={`portal-search-card__price price-tag${row.sell_price == null ? " price-tag--muted" : ""}`}>
                    ...
                    {catalogResults.length && effectivePortalSearchView === "list" ? (
                      <DataTable
                        ...
                        className="data-table--portal-catalog"
                        wrapClassName="table-wrap--records"
                  ...
                  {portalRecommendationCards.length ? (
                    <SectionCard title={`Related Alternatives (${portalRecommendationCards.length.toLocaleString("en-US")})`}>
                      <div className="portal-inline-note portal-inline-note--soft">
                        <span>Alternative Logic</span>
                        <strong>These rows are held outside the direct hit list and ranked from the same product family plus current stock movement.</strong>
                      </div>
                      <div className="portal-search-card-grid">
                  ...
                  <div ref={portalDraftLinesRef}>
                    <SectionCard title={`Basket (${portalDraftLines.length.toLocaleString("en-US")})`}>
                      <div className="portal-basket-notes-row">
                        <Input label="Basket Notes" value={portalOrderNotes} placeholder="Internal note for purchasing or dispatch" onChange={setPortalOrderNotes} />
                      </div>
                      <DataTable
                        rows={portalDraftLines}
                        columns={portalDraftColumns}
                        className="data-table--portal-draft"
                        wrapClassName="table-wrap--records"
            ...
            <div className="workbench-detail-list">
              {portalPreview.kind === "basket" ? (
                ...
                <>
                  ...
                  <div><span>Vehicle Model</span><strong>{portalPreview.item.vehicle_model || "-"}</strong></div>
