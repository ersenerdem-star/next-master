...
import { shouldUseCPriceForCustomer, type CustomerPricingMode } from "../../../shared/pricingPolicy";
...
const SALES_ORDER_IMPORT_CHUNK_SIZE = 40;
...
function isMeaningfulSalesOrderWorkspaceCache(cache: PersistedSalesOrderWorkspace | null) {
  if (!cache) return false;
  if (cache.selectedLocalSalesOrderId) return true;
  if (cache.workbenchMode !== "new") return true;
  if (Array.isArray(cache.quoteBuilderLines) && cache.quoteBuilderLines.length > 0) return true;
  return Boolean(
    String(cache.customerName || "").trim() ||
      String(cache.customerSelection || "").trim() ||
      String(cache.manualCustomerName || "").trim() ||
      String(cache.quoteNotes || "").trim() ||
      String(cache.buyerInfo || "").trim() ||
      String(cache.deliveryTerm || "").trim() ||
      String(cache.paymentTerms || "").trim() ||
      String(cache.packingDetails || "").trim() ||
      String(cache.quoteBrand || "").trim() ||
      String(cache.quoteBrandSelection || "").trim() ||
      String(cache.salesOrderSourceSnapshot || "").trim(),
  );
}

function hasSalesOrderEditorWorkspace(cache: PersistedSalesOrderWorkspace | null) {
  if (!cache) return false;
  if (cache.selectedLocalSalesOrderId || cache.selectedQuoteId) return true;
  if (cache.salesOrdersView === "detail" && cache.workbenchMode === "new") return true;
  if (cache.workbenchMode !== "new") return false;
  return Boolean(
    Array.isArray(cache.quoteBuilderLines) && cache.quoteBuilderLines.length > 0 ||
      String(cache.quoteNo || "").trim() ||
      String(cache.customerName || "").trim() ||
      String(cache.customerSelection || "").trim() ||
      String(cache.manualCustomerName || "").trim() ||
      String(cache.quoteNotes || "").trim() ||
...
function buildDraftQuoteHtml(input: {
  ...
  return buildBusinessDocumentHtml({
    ...
    totalQty,
    totalWeight,
    showOrderNoColumn: input.showOrderNoColumn ?? true,
  });
}
...
export function QuotesPage({
  ...
  const initialWorkspaceCacheRaw = typeof window === "undefined" ? null : readSalesOrderWorkspaceCache();
  const initialWorkspaceCache = isMeaningfulSalesOrderWorkspaceCache(initialWorkspaceCacheRaw) ? initialWorkspaceCacheRaw : null;
  const hasRestorableEditorWorkspace = hasSalesOrderEditorWorkspace(initialWorkspaceCache);
  ...
  const [salesOrdersView, setSalesOrdersView] = useState<"list" | "detail">(
    externalSelectedQuoteId || externalSelectedSalesOrderId ? "detail" : hasRestorableEditorWorkspace ? "detail" : initialWorkspaceCache?.salesOrdersView || "list",
  );
  ...
  const syncedCustomerPricingRef = useRef("");
  ...
  useEffect(() => {
    if (workbenchMode === "existing" && selectedLocalSalesOrderId) {
      return;
    }
    if (customerSelection === "__manual__") {
      syncedCustomerPricingRef.current = "";
      return;
    }
    const syncKey = String(selectedCustomerProfile?.id || customerSelection || "").trim();
    if (!syncKey || syncedCustomerPricingRef.current === syncKey) return;
    syncedCustomerPricingRef.current = syncKey;
    if (selectedCustomerProfile?.price_list_type) {
      setCustomerType(selectedCustomerProfile.price_list_type);
    }
  }, [customerSelection, selectedCustomerProfile?.id, selectedCustomerProfile?.price_list_type, selectedLocalSalesOrderId, workbenchMode]);
  ...
  useEffect(() => {
    ...
    async function run() {
      if (!externalSelectedQuoteId && !submittedSearch.trim() && !initialCachedQuoteId) {
        setQuotes([]);
  ...
  }, [externalSelectedQuoteId, initialCachedQuoteId, submittedSearch]);
  ...
  useEffect(() => {
    ...
    workspaceCacheWriteTimeoutRef.current = window.setTimeout(() => {
      writeSalesOrderWorkspaceCache(isMeaningfulSalesOrderWorkspaceCache(snapshot) ? snapshot : null);
  ...
  useEffect(() => {
    ...
    const flushWorkspaceCache = () => {
      writeSalesOrderWorkspaceCache(isMeaningfulSalesOrderWorkspaceCache(workspaceCacheSnapshotRef.current) ? workspaceCacheSnapshotRef.current : null);
    };
  ...
  useEffect(() => {
    ...
    if (
      ...
      (workbenchMode === "new" ||
        Boolean(selectedLocalSalesOrderId) ||
        Boolean(selectedQuoteId) ||
        quoteBuilderLines.length > 0 ||
        customerName.trim() ||
  ...
  async function maybeAcknowledgePortalOrder(order: LocalSalesOrder | null) {
    if (!order || order.source_channel !== "portal" || !order.portal_submitted_at || order.portal_seen_at) {
      return;
    }
    const shouldMarkAsHandled = window.confirm(
      `${order.sales_order_no || "This portal order"} still appears as a new portal order on Home. Click OK if you handled it and want to clear the Home alert. Click Cancel to keep the alert active.`,
    );
    ...
    try {
      ...
      if (selectedLocalSalesOrderId === seenOrder.id) {
        setSalesOrderSourceSnapshot(serializeSalesOrderForDirtyCheck(seenOrder));
      }
      actionFeedback.succeed(`${seenOrder.sales_order_no} marked as reviewed.`);
    } catch (caught) {
      actionFeedback.fail(caught instanceof Error ? caught.message : "Portal order review update failed");
    }
  }

  async function confirmSalesOrderNavigation(nextAction: () => void | Promise<void>) {
    let portalOrderToAcknowledge = currentLocalSalesOrder;
    if (!salesOrderHasUnsavedChanges) {
      await maybeAcknowledgePortalOrder(portalOrderToAcknowledge);
      await nextAction();
      return;
    }
    ...
    try {
      ...
      if (!saved) return;
      portalOrderToAcknowledge = saved;
  ...
  function handlePrintDraftPdf() {
    ...
    const html = buildDraftQuoteHtml({
      quoteNo,
      quoteDate,
  ...
  function handleDownloadDraftPdf() {
    if (!quoteBuilderLines.length) {
      actionFeedback.fail("Add quote lines first.");
      return;
    }
    ...
    try {
      actionFeedback.begin(`Preparing PDF download for ${quoteNo || "sales order draft"}...`);
      void downloadPdfFromHtml({
        html,
        filename: `${(quoteNo || "sales-order-draft").replace(/[^a-z0-9_-]+/gi, "-")}.pdf`,
      })
        .then(() => actionFeedback.succeed("PDF downloaded."))
        .catch((caught) => {
          actionFeedback.fail(caught instanceof Error ? caught.message : "PDF download failed");
        })
        .finally(() => setDownloadingDraftPdf(false));
    } catch (caught) {
      actionFeedback.fail(caught instanceof Error ? caught.message : "PDF download failed");
  ...
  async function refreshLocalSalesOrders(nextSelectedId?: string) {
    ...
    void fetchSalesOrderBrandSummaries()
      .then((brandSummaryRows) => {
        setSalesOrderBrandsById(Object.fromEntries(brandSummaryRows.map((row) => [row.id, row.brands])));
      })
      .catch(() => {
        setSalesOrderBrandsById({});
      });
  ...
  const filteredLocalSalesOrders = useMemo(() => {
    ...
    return localSalesOrders.filter((order) => {
      ...
      const brandSummary = salesOrderBrandsById[order.id] || [];
      ...
      return includesLooseText(haystack, needle);
    });
  }, [localSalesOrders, salesOrderBrandsById, salesOrderDocumentState, salesOrderFilter, search]);
  ...
  function handleSalesOrderListSearch() {
    setSearchingQuotes(false);
    const trimmedSearch = search.trim();
    actionFeedback.succeed(
      `${filteredLocalSalesOrders.length.toLocaleString("en-US")} sales order(s) matched${trimmedSearch ? ` for ${trimmedSearch}` : ""}.`,
    );
  }
  ...
  const savedSalesOrderColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "Sales Order",
        render: (row: LocalSalesOrder) => (
          <span className="sales-order-no" title={row.sales_order_no}>
            {row.sales_order_no}
          </span>
        ),
        ...
        <div className="quote-list-panel__header">
          ...
          <div className="toolbar toolbar--wrap">
            <Input
              ...
              onEnter={handleSalesOrderListSearch}
            />
            <Button
              onClick={handleSalesOrderListSearch}
            >
              Search
            </Button>
          </div>
          ...
          {!!filteredLocalSalesOrders.length ? (
            <div className="quote-list-section">
              ...
              <DataTable
                ...
                className="data-table--sales-orders"
      ...
      <section className="quote-editor-panel">
        <div className="quote-editor-panel__header">
          ...
          <div className="toolbar toolbar--wrap">
            ...
            <Button variant="secondary" onClick={handleDownloadDraftPdf} busy={downloadingDraftPdf} busyLabel="Preparing...">
              Download PDF
            </Button>
            {quoteBuilderLines.length ? (
              <>
        ...
        <div className="modal-backdrop" onClick={() => setQuoteLinePreview(null)}>
          <DraggableSurface className="modal-card" dragHandleSelector=".draggable-surface__handle" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="workbench-detail-list">
              ...
              <div><span>Buy</span><strong className="price-tag price-tag--compact price-tag--buy">{formatMoney(quoteLinePreview.buy_price, currency)}</strong></div>
              <div><span>Sell</span><strong className="price-tag price-tag--compact">{formatMoney(quoteLinePreview.sell_price, currency)}</strong></div>
