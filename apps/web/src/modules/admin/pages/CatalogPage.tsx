...
import { syncBrandCatalog, type SyncBrandCatalogResult } from "../../../infrastructure/api/adminApi";
...
import { ProductVisual, type ProductMediaItem } from "../../../presentation/components/common/ProductVisual";
...
import { CATALOG_MARKET_SEGMENT_OPTIONS, formatCatalogMarketSegmentLabel, normalizeCatalogMarketSegment } from "../../../shared/catalogSegments";
...
const CATALOG_LAST_VISIBLE_ROWS_KEY = "next-master-catalog-last-visible-rows";
...
type CatalogOfflineCache = {
  ...
  lastSyncResult?: SyncBrandCatalogResult | null;
  updatedAt: string;
};

function formatExecutionProfile(value?: string) {
  switch (value) {
    case "official_first":
      return "Official first";
    case "official_tecalliance_primary":
      return "Official + TecAlliance primary";
    case "official_blocked_browser_capture":
      return "Official blocked, browser capture required";
    case "temporary_marketplace_fallback":
      return "Temporary marketplace fallback";
...
function readLastVisibleCatalogRows() {
  if (typeof window === "undefined") return [] as CatalogRow[];
  try {
    const raw = window.localStorage.getItem(CATALOG_LAST_VISIBLE_ROWS_KEY);
    if (!raw) return [] as CatalogRow[];
...
function writeLastVisibleCatalogRows(rows: CatalogRow[]) {
  if (typeof window === "undefined") return;
  if (!rows.length) return;
  window.localStorage.setItem(CATALOG_LAST_VISIBLE_ROWS_KEY, JSON.stringify(rows));
}
...
function buildCatalogCacheSnapshot(params: {
  ...
  return {
    ...
    catalogSegment: params.catalogSegment || "",
    submittedCatalogSegment: params.submittedCatalogSegment || "",
    selectedCatalogProductId: params.selectedCatalogProductId,
    lastSyncResult: params.lastSyncResult ?? existing?.lastSyncResult ?? null,
...
function filterCachedCatalogRows(rows: CatalogRow[], search: string, brand: string, marketSegment: string) {
  ...
  const filtered = rows.filter((row) => {
    ...
    const rawMatch = [
      ...
      row.origin,
      row.market_segment,
      row.market_segment ? formatCatalogMarketSegmentLabel(row.market_segment) : "",
    ...
    if (normalizedSearch) {
      const normalizedFields = [row.product_code, row.ean, row.oem_no, row.replacement_old_code, row.replacement_code].map((value) => normalizePartCode(String(value || "")));
...
function isSoftCatalogRequestFailure(error: unknown) {
  const normalized = String(error instanceof Error ? error.message : error || "").toLowerCase();
  return (
    normalized.includes("timed out") ||
    normalized.includes("request could not be completed right now") ||
    normalized.includes("request took too long") ||
    normalized.includes("request failed") ||
    normalized.includes("failed to fetch") ||
    normalized.includes("networkerror") ||
    normalized.includes("load failed") ||
    normalized.includes("app rpc failed") ||
    normalized.includes("rpc cloud_catalog_page") ||
    normalized.includes("brand lookup failed") ||
    normalized.includes("statement timeout") ||
    normalized.includes("catalog request failed")
  );
}
...
export function CatalogPage() {
  ...
  const [lastSyncResult, setLastSyncResult] = useState<SyncBrandCatalogResult | null>(null);
  const lastSuccessfulCatalogRowsRef = useRef<CatalogRow[]>([]);
  ...
  const marketSegmentFilterOptions = [{ value: "", label: "All Segments" }, ...CATALOG_MARKET_SEGMENT_OPTIONS];
  const marketSegmentFormOptions = [{ value: "", label: "Unspecified" }, ...CATALOG_MARKET_SEGMENT_OPTIONS];

  function rememberCatalogRows(nextRows: CatalogRow[]) {
    if (nextRows.length) {
      lastSuccessfulCatalogRowsRef.current = nextRows;
      writeLastVisibleCatalogRows(nextRows);
    }
  }
  ...
  useEffect(() => {
    ...
    const lastVisibleRows = readLastVisibleCatalogRows();
    if (!cached.rows.length && lastVisibleRows.length) setRows(lastVisibleRows);
    ...
    setLastSyncResult(cached.lastSyncResult || null);
    if (!isOnline) {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        if (previewSelection && !submittedSearch.trim() && submittedCatalogBrand === previewSelection.brand) {
          try {
            result = await fetchCatalogRowsByCodes({
              brandName: previewSelection.brand,
              codes: previewSelection.codes,
            });
          } catch (caught) {
            console.warn("Catalog preview code lookup failed, falling back to catalog search.", caught);
            result = await loadCatalogRowsForFilters(previewSelection.codes[0] || "", previewSelection.brand, submittedCatalogSegment);
          }
        } else {
          result = await loadCatalogRowsForFilters(submittedSearch, submittedCatalogBrand, submittedCatalogSegment);
        }
        if (!cancelled) {
          setRows(result);
          rememberCatalogRows(result);
        }
      } catch (caught) {
        if (cancelled) return;
        const cached = readCatalogCache();
        const cachedRows = cached?.rows || [];
        const fallbackRows = cachedRows.length ? filterCachedCatalogRows(cachedRows, submittedSearch, submittedCatalogBrand, submittedCatalogSegment) : [];
        if (isSoftCatalogRequestFailure(caught)) {
          if (fallbackRows.length) {
            setRows(fallbackRows);
            rememberCatalogRows(fallbackRows);
            setStatus(`Catalog service was slow. Showing ${fallbackRows.length.toLocaleString("en-US")} cached row(s); retry for fresh data.`);
          } else if (lastSuccessfulCatalogRowsRef.current.length) {
            setRows(lastSuccessfulCatalogRowsRef.current);
            setStatus(
              `Catalog service was slow. Preserving the last visible result set (${lastSuccessfulCatalogRowsRef.current.length.toLocaleString("en-US")} row(s)); retry in a moment.`,
            );
          } else {
            const lastVisibleRows = readLastVisibleCatalogRows();
            if (lastVisibleRows.length) {
              setRows(lastVisibleRows);
              setStatus(
                `Catalog service was slow. Preserving the last visible result set (${lastVisibleRows.length.toLocaleString("en-US")} row(s)); retry in a moment.`,
              );
            } else {
              setStatus("Catalog service was slow. Preserving the last visible result set; retry in a moment.");
            }
          }
          setError("");
        } else {
  ...
  useEffect(() => {
    ...
    const missingRows = visibleRows.filter((row) => {
      const key = `${row.brand.trim().toLowerCase()}::${normalizePartCode(row.product_code)}`;
      return referenceCoverage[key] == null;
    });
  ...
  }, [brands, rows, drafts, search, submittedSearch, catalogBrand, submittedCatalogBrand, catalogSegment, submittedCatalogSegment, selectedCatalogProductId, lastSyncResult]);
  ...
  const catalogCountLabel = loading
    ...
    : hasSubmittedBrand && hasSubmittedSegment && !hasSubmittedSearch
      ? `${submittedCatalogBrand} / ${formatCatalogMarketSegmentLabel(submittedCatalogSegment)}: ${visibleTotal.toLocaleString("en-US")}${hasApproximateTotal ? "+" : ""} items`
    ...
    : hasSubmittedSegment && !hasSubmittedSearch && !hasSubmittedBrand
  ...
  async function loadCatalogRowsForFilters(nextSearch: string, nextBrand: string, nextSegment: string) {
    return await fetchCloudCatalog({
      search: nextSearch,
      brandName: nextBrand,
      ...
      const cached = readCatalogCache();
      const cachedRows = cached?.rows || [];
      const fallbackRows = cachedRows.length ? filterCachedCatalogRows(cachedRows, nextSearch, nextBrand, nextSegment) : [];
      if (fallbackRows.length) {
        setStatus(`Catalog service was slow. Showing ${fallbackRows.length.toLocaleString("en-US")} cached row(s); retry for fresh data.`);
        setError("");
        return fallbackRows;
  ...
  function applyCatalogFilters(nextSearch: string, nextBrand: string, nextSegment: string, announce = true) {
    setSearchingCatalog(true);
    setPreviewSelection(null);
    setSelectedCatalogProductId("");
    if (announce) {
      const segmentLabel = nextSegment ? formatCatalogMarketSegmentLabel(nextSegment) : "all segments";
      actionFeedback.begin(
        `${isOnline ? "Searching" : "Filtering cached"} catalog for ${nextBrand || "all brands"} / ${segmentLabel} / ${nextSearch.trim() || "all items"}...`,
      );
    }
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (row: CatalogRow) => (
          <div className="catalog-cell catalog-cell--stack">
            <span className="catalog-mono catalog-clip">{drafts[row.product_id]?.ean ?? row.ean ?? "-"}</span>
          </div>
        ),
      },
      {
  ...
  async function reloadCatalog(nextSearch = submittedSearch, nextBrand = submittedCatalogBrand, nextSegment = submittedCatalogSegment) {
    ...
    setStatus("");
    try {
      const result = await loadCatalogRowsForFilters(nextSearch, nextBrand, nextSegment);
      setRows(result);
      rememberCatalogRows(result);
    } catch (caught) {
      const cached = readCatalogCache();
  ...
  async function handleCatalogImport(file: File) {
    ...
    try {
      ...
      const eanIndex = indexOfAny("EAN", "GTIN", "Barcode", "EAN_13", "EAN13");
      ...
      const vehicleModelIndex = indexOfAny("Vehicle_Model", "Vehicle Model", "Model", "Vehicle_Application_Model");
      const originIndex = indexOfAny("Origin", "Country_Of_Origin");
      const weightIndex = indexOfAny("Weight_kg", "Weight", "Net_Weight");
      const marketSegmentIndex = indexOfAny("Market_Segment", "Market Segment", "Product_Segment", "Product Segment", "Segment", "Vehicle_Segment");
      ...
      const payload = dataRows
        .map((row) => ({
          ...
          market_segment: normalizeCatalogMarketSegment(normalizeText(row[marketSegmentIndex])),
  ...
  async function handleCatalogExport() {
    ...
    try {
      ...
      const exportRows = buildCatalogExportRows(exportData);
  ...
  async function handleCatalogExportExcel() {
    if (!isOnline) {
      setError("Connect to the internet to export catalog data.");
      return;
    }
    ...
    setError("");
    setStatus("");
    actionFeedback.begin(`Preparing catalog Excel export for ${exportBrand}...`);

    try {
      const exportData = await fetchCatalogExportRows({ brandName: exportBrand });
      const blob = buildXlsxBlob(`${exportBrand} Catalog`, buildCatalogExportRows(exportData), [9]);
      downloadBlob(`catalog-${exportBrand.toLowerCase().replace(/\s+/g, "-")}.xlsx`, blob);
      setShowExportDialog(false);
      setStatus(`Catalog Excel downloaded for ${exportBrand}.`);
      actionFeedback.succeed(`Catalog Excel downloaded for ${exportBrand}.`);
    } catch (caught) {
      const message = caught instanceof Error ? caught.message : "Catalog export failed";
      setError(message);
      actionFeedback.fail(message);
  ...
  function buildCatalogExportRows(exportData: Awaited<ReturnType<typeof fetchCatalogExportRows>>) {
    return [
      ["Product_Code", "Brand", "Product_Name", "EAN", "OEM_No", "Vehicle", "Vehicle_Model", "HS_Code", "Origin", "Weight_kg", "Market_Segment", "Image_URL", "Lifecycle_Status", "Lifecycle_Note"],
      ...exportData.map((row) => [
        row.product_code,
        row.brand,
        row.description || "",
  ...
  return (
    <div className="page-stack">
      <section className="section-card search-focus-card search-focus-card--admin catalog-workbench">
        <div className="section-card__header section-card__header--row catalog-workbench__header">
          <div className="catalog-workbench__intro">
          ...
          <div className="catalog-command-deck">
            <div className="catalog-command-deck__search">
              <Select
                value={catalogBrand}
              ...
              <Select
                ...
                options={marketSegmentFilterOptions}
                onChange={(value) => {
                  setCatalogSegment(value);
                  if (value || catalogBrand || search.trim()) {
            ...
            <div className="catalog-command-deck__actions">
              <Button
                variant="secondary"
                onClick={clearCatalogSearch}
                disabled={!search && !submittedSearch && !catalogBrand && !submittedCatalogBrand && !catalogSegment && !submittedCatalogSegment && !rows.length}
              >
                Clear Search
              </Button>
        ...
        <div className="section-card__body section-card__body--catalog">
          <div className="catalog-results-surface">
            ...
            {lastSyncResult ? (
              <div className="attention-panel catalog-sync-summary">
                <div className="attention-panel__header">
                  <div>
                    <span className="search-focus-card__eyebrow">Last Brand Sync</span>
                    <div className="catalog-sync-summary__title">{lastSyncResult.targetBrandName}</div>
                  </div>
                  <div className="document-marks document-marks--compact">
                    <span className={`mark-badge ${lastSyncResult.fallbackUsed ? "mark-badge--accent" : "mark-badge--success"}`}>
                      {lastSyncResult.fallbackUsed ? "Fallback active" : "Preferred source active"}
                    </span>
                    <span className="mark-badge mark-badge--info">{lastSyncResult.sourcePolicyVersion || "Policy n/a"}</span>
                  </div>
                </div>
                <div className="catalog-sync-summary__grid">
                  <div className="catalog-sync-summary__item">
                    <span className="catalog-sync-summary__label">Preferred Source</span>
                    <span className="catalog-sync-summary__value">{lastSyncResult.preferredProviderLabel}</span>
                    <a className="catalog-sync-summary__link" href={lastSyncResult.preferredSourceUrl} target="_blank" rel="noreferrer">
                      Open source
                    </a>
                  </div>
                  <div className="catalog-sync-summary__item">
                    <span className="catalog-sync-summary__label">Execution Path</span>
                    <span className="catalog-sync-summary__value">{lastSyncResult.executionProviderLabel}</span>
                    <span className="catalog-sync-summary__hint">
                      {lastSyncResult.executionSourceType === "official" ? "Official execution" : "Fallback execution"}
                    </span>
                  </div>
                  <div className="catalog-sync-summary__item">
                    <span className="catalog-sync-summary__label">Execution Profile</span>
                    <span className="catalog-sync-summary__value">{formatExecutionProfile(lastSyncResult.sourcePolicy?.executionProfile)}</span>
                    <span className="catalog-sync-summary__hint">
                      Primary authority: {lastSyncResult.sourcePolicy?.primaryAuthority.label || lastSyncResult.preferredProviderLabel}
                    </span>
                  </div>
                  <div className="catalog-sync-summary__item">
                    <span className="catalog-sync-summary__label">Coverage</span>
                    <div className="catalog-sync-summary__chips">
                      <span className="mark-badge mark-badge--success">{lastSyncResult.resolvedRows.toLocaleString("en-US")} synced</span>
                      <span className="mark-badge">{lastSyncResult.newRowsInListing.toLocaleString("en-US")} new</span>
                      <span className="mark-badge mark-badge--info">{lastSyncResult.replacementRows.toLocaleString("en-US")} replacements</span>
                      <span className="mark-badge mark-badge--danger">{lastSyncResult.errorRows.toLocaleString("en-US")} errors</span>
                    </div>
                  </div>
                  {lastSyncResult.sourcePolicy?.helperSources?.length ? (
                    <div className="catalog-sync-summary__item">
                      <span className="catalog-sync-summary__label">Helper Sources</span>
                      <div className="catalog-sync-summary__chips">
                        {lastSyncResult.sourcePolicy.helperSources.map((source) => (
                          <span key={source.key} className="mark-badge">
                            {source.label}
                          </span>
                        ))}
                      </div>
                  ...
                  {lastSyncResult.sourcePolicy?.completionSources?.length ? (
                    <div className="catalog-sync-summary__item">
                      <span className="catalog-sync-summary__label">Completion Sources</span>
                      <div className="catalog-sync-summary__chips">
                        {lastSyncResult.sourcePolicy.completionSources.map((source) => (
                          <span key={source.key} className="mark-badge mark-badge--accent">
                            {source.label}
                          </span>
                        ))}
                      </div>
                ...
                {lastSyncResult.sourcePolicy?.operationalNotes?.length ? (
                  <div className="catalog-sync-summary__notes">
                    {lastSyncResult.sourcePolicy.operationalNotes.map((note) => (
                      <div key={note} className="catalog-sync-summary__note">
                        {note}
                      </div>
                    ))}
                  </div>
            ...
            <div className="workbench-main-layout catalog-workbench-layout">
              <div className="workbench-main-layout__table">
                <DataTable
                  ...
                  rows={rows}
                  columns={columns}
                  emptyText={loading ? "Loading..." : !submittedSearch.trim() && !submittedCatalogBrand && !submittedCatalogSegment ? "Select a brand or search to load catalog." : "No products found"}
      ...
      {selectedCatalogRow && selectedCatalogDraft ? (
        <div className="modal-backdrop modal-backdrop--catalog-detail" onClick={() => setSelectedCatalogProductId("")}>
          <DraggableSurface
            className="catalog-selected-popup"
            ref={selectedCatalogPopupRef}
          ...
          >
            ...
            <div className="document-marks document-marks--compact">
              ...
              {selectedCatalogDraft.market_segment ? <span className="mark-badge mark-badge--info">{formatCatalogMarketSegmentLabel(selectedCatalogDraft.market_segment)}</span> : null}
            ...
            <div className="catalog-detail-editor">
              ...
              <Select
                label="Segment"
                value={selectedCatalogDraft.market_segment || ""}
                options={marketSegmentFormOptions}
            ...
            <div className="workbench-detail-list">
              ...
              <div>
                <span>EAN</span>
                <strong>{selectedCatalogDraft.ean || "-"}</strong>
              </div>
              <div>
                <span>OEM</span>
              ...
              <div><span>Vehicle Model</span><strong>{selectedCatalogDraft.vehicle_model || "-"}</strong></div>
        ...
        <div className="modal-backdrop">
          ...
          <div className="modal-hint">This export ignores the current search box and downloads the full catalog of the selected brand.</div>
            <div className="modal-actions">
              ...
              <Button onClick={() => void handleCatalogExportExcel()} disabled={!exportBrand} busy={exportingCatalog} busyLabel="Preparing...">
                Export Excel
              </Button>
            </div>
          </DraggableSurface>
        ...
        <div className="modal-backdrop">
          <DraggableSurface className="modal-card" dragHandleSelector=".draggable-surface__handle">
            ...
            <div className="modal-form-grid">
              ...
              <Input label="EAN" value={createDraft.ean} onChange={(value) => setCreateDraft((current) => ({ ...current, ean: value }))} />
              ...
              <Input label="Vehicle Model" value={createDraft.vehicle_model} onChange={(value) => setCreateDraft((current) => ({ ...current, vehicle_model: value }))} />
              ...
              <Select
                ...
                options={marketSegmentFormOptions}
                onChange={(value) => setCreateDraft((current) => ({ ...current, market_segment: normalizeCatalogMarketSegment(value) || "" }))}
              />
              <Select
                label="Lifecycle"
