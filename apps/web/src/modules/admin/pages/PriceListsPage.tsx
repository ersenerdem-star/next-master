...
import {
  fetchBrandMarginPriceSummaries,
  fetchCPriceExportFallbackRows,
  fetchPriceListSettings,
  fetchSupplierPriceExportFallbackRows,
  importCPriceList,
  updateMarginPriceList,
} from "../../../infrastructure/api/priceListsApi";
...
function isOldCodeCoverageTimeoutError(error: unknown) {
  const message = sanitizeUserFacingMessage(error, "");
...
function isSoftLookupFailure(error: unknown) {
  const message = sanitizeUserFacingMessage(error, "");
  const normalized = message.toLowerCase();
  return (
    !normalized ||
    isOldCodeCoverageTimeoutError(error) ||
    normalized.includes("could not be completed") ||
...
function formatPriceListError(error: unknown, fallback = "Price list download failed") {
...
export function PriceListsPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        if (!cancelled) {
          setBrands([]);
          setStatus(formatPriceListError(caught, "Price list settings load failed"));
        }
      }
    }
  ...
  async function handleSaveMargins() {
    ...
    try {
      ...
      const message = formatPriceListError(caught, "Margin update failed");
      setStatus(message);
  ...
  async function handleImportCPriceList() {
    ...
    try {
      ...
      const message = formatPriceListError(caught, "C price import failed");
      setStatus(message);
      actionFeedback.fail(message);
    } finally {
      setImportingC(false);
  ...
  async function handleDownloadPriceList() {
    ...
    try {
      ...
      if (downloadListType === "C") {
        const [catalogRows, cPriceRows] = await Promise.all([
          fetchCatalogExportRows({ brandName: downloadBrand }).catch(() => []),
          fetchCPriceExportFallbackRows({ brandName: downloadBrand }),
        ]);
        if (!cPriceRows.length) {
          throw new Error(`No C price rows were available for ${downloadBrand}. Price list download was not created.`);
        }
        const catalogByCode = new Map(catalogRows.map((row) => [normalizePartCode(row.product_code), row]));
        const oldCodesByNewCode: Record<string, string[]> = await fetchOldCodesByNewCodeForBrand({
          brand: downloadBrand,
          newCodes: cPriceRows.map((row) => row.product_code),
        }).catch((caught) => {
          if (isSoftLookupFailure(caught)) {
        ...
        sheetRows.push(
          ...cPriceRows.map((priceRow) => {
            const catalogRow = catalogByCode.get(priceRow.normalized_code);
            return [
              priceRow.product_code,
              (oldCodesByNewCode[priceRow.normalized_code] || []).join(" | "),
              catalogRow?.brand || downloadBrand,
              catalogRow?.description || "",
              catalogRow?.oem_no || "",
              catalogRow?.hs_code || "",
              catalogRow?.origin || "",
              catalogRow?.weight_kg ?? "",
              `${downloadListType} Price List`,
              priceRow.sellPrice,
              priceRow.priceDate ? `C price updated ${priceRow.priceDate}` : "",
            ];
          }),
        );
      } else {
        ...
        if (!catalogRows.length) {
          const fallbackRows = await fetchSupplierPriceExportFallbackRows({ brandName: downloadBrand, marginPercent });
          if (!fallbackRows.length) {
            throw new Error(`No catalog or supplier price rows were available for ${downloadBrand}. Price list download was not created.`);
          }
          oldCodeCoverageTimedOut = true;
          sheetRows.push(
            ...fallbackRows.map((row) => [
              ...
              `${downloadListType} Price List`,
              row.salesPrice ?? "",
              row.notes ? `${row.notes} | Catalog metadata fallback` : "Catalog metadata fallback",
            ]),
          );
          ...
          setStatus(`${downloadListType} price list downloaded for ${downloadBrand}. Catalog rows were unavailable, so supplier price rows were used.`);
      ...
      if (sheetRows.length <= 1) {
        throw new Error(`No downloadable rows were available for ${downloadBrand}.`);
      }
      const blob = buildXlsxBlob(`${downloadBrand} ${downloadListType}`, sheetRows, [8, 10, 13]);
      downloadBlob(`price-list-${downloadBrand.toLowerCase().replace(/\s+/g, "-")}-${downloadListType.toLowerCase()}.xlsx`, blob);
      setStatus(
      ...
      const message = formatPriceListError(caught, "Price list download failed");
      setStatus(message);
      actionFeedback.fail(message);
    } finally {
  ...
  return (
    <SectionCard title="Price Lists">
      ...
      {status ? (
        <div className={/\b(updated|imported|downloaded|saved)\b/i.test(status) ? "success-text" : "error-text"}>{status}</div>
      ) : null}
