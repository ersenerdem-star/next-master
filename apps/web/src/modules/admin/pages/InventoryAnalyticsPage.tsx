...
function normalizeKey(brand: string, code: string) {
  return `${brand.trim().toLowerCase()}::${code.trim().toLowerCase().replace(/[^a-z0-9]+/g, "")}`;
}
...
function buildPurchaseCoverageMap(purchaseOrders: LocalPurchaseOrder[]) {
  ...
  purchaseOrders.forEach((purchaseOrder) => {
    purchaseOrder.lines.forEach((line) => {
      ...
      const keys = [
        `${purchaseOrder.sales_order_id}::${normalizeKey(line.brand || "", line.product_code || "")}`,
        `${purchaseOrder.sales_order_id}::${normalizeKey(line.brand || "", line.old_code || "")}`,
      ];
      [...new Set(keys)].forEach((key) => {
        coverage.set(key, round((coverage.get(key) || 0) + qty));
      });
    });
  });
  return coverage;
...
export function InventoryAnalyticsPage({ onOpenSalesOrder, onOpenInventoryWarehouse, onOpenInventoryItem }: InventoryAnalyticsPageProps) {
  ...
  const turnoverRows = useMemo<TurnoverRow[]>(() => {
    ...
    filteredStockItems.forEach((item) => {
      ...
      current.reserved_qty += toNumber(item.reserved_qty);
      current.available_qty += toNumber(item.available_qty);
    ...
    return filteredStockItems
      .map((item) => {
        ...
        return {
          ...
          reserved_qty: round(toNumber(item.reserved_qty)),
          available_qty: round(toNumber(item.available_qty)),
    ...
    return Array.from(allKeys)
      .map((key) => {
        ...
        return {
          ...
          reserved_qty: round(filteredStockItems.filter((item) => normalizeKey(item.brand, item.product_code || item.old_code) === key).reduce((sum, item) => sum + toNumber(item.reserved_qty), 0)),
          available_qty: round(filteredStockItems.filter((item) => normalizeKey(item.brand, item.product_code || item.old_code) === key).reduce((sum, item) => sum + toNumber(item.available_qty), 0)),
    ...
    return salesOrders
      ...
      .flatMap((order) =>
        order.lines.map((line) => {
          ...
          const lookupKeys = [
            `${order.id}::${normalizeKey(line.brand || "", line.resolvedCode || "")}`,
            `${order.id}::${normalizeKey(line.brand || "", line.requestedCode || "")}`,
          ];
          const purchasedQty = round(Math.max(...lookupKeys.map((key) => purchaseCoverage.get(key) || 0), 0));
  ...
  const turnoverColumns = useMemo(
    () => [
      ...
      { key: "reserved", header: "Reserved", render: (row: TurnoverRow) => row.reserved_qty.toLocaleString("en-US") },
      { key: "available", header: "Available", render: (row: TurnoverRow) => row.available_qty.toLocaleString("en-US") },
  ...
  const agingColumns = useMemo(
    () => [
      ...
      { key: "reserved", header: "Reserved", render: (row: AgingRow) => row.reserved_qty.toLocaleString("en-US") },
      { key: "available", header: "Available", render: (row: AgingRow) => row.available_qty.toLocaleString("en-US") },
  ...
  async function handleExport() {
    ...
    try {
      ...
      if (activeTab === "Turnover") {
        rows = [
          ["Brand", "Code", "Description", "On_Hand_Qty", "Reserved_Qty", "Available_Qty", "Stock_Value_EUR", "Sold_Qty", "Sold_Amount_EUR", "Turnover_X", "Days_Cover"],
          ...turnoverRows.map((row) => [row.brand, row.product_code, row.description, row.on_hand_qty, row.reserved_qty, row.available_qty, row.stock_value, row.sold_qty, row.sold_amount, row.turnover_ratio, row.days_cover ?? ""]),
        ];
        numericColumns = [3, 4, 5, 6, 7, 8, 9, 10];
      } else if (activeTab === "Aging") {
        rows = [
          ["Warehouse", "Brand", "Code", "Description", "On_Hand_Qty", "Reserved_Qty", "Available_Qty", "Stock_Value_EUR", "Days_Idle", "Age_Bucket", "Last_Move"],
          ...agingRows.map((row) => [row.warehouse_name, row.brand, row.product_code, row.description, row.on_hand_qty, row.reserved_qty, row.available_qty, row.stock_value, row.days_idle, row.age_bucket, formatDate(row.last_moved_at)]),
        ];
        numericColumns = [4, 5, 6, 7, 8];
      } else if (activeTab === "Forecast") {
        rows = [
          ["Brand", "Code", "Description", "On_Hand_Qty", "Reserved_Qty", "Available_Qty", "Monthly_Demand_Qty", "Months_Cover", "Recommended_Qty", "Reorder_Qty", "Status"],
          ...forecastRows.map((row) => [row.brand, row.product_code, row.description, row.on_hand_qty, row.reserved_qty, row.available_qty, row.monthly_demand_qty, row.months_cover ?? "", row.recommended_qty, row.reorder_qty, row.status]),
        ];
          ...
          {activeTab === "Turnover" ? (
            <>
              <div className="settings-grid settings-stats-grid">
                ...
                <div className="settings-item">
                  <span className="settings-label">Reserved Qty</span>
                  <strong>{turnoverRows.reduce((sum, row) => sum + row.reserved_qty, 0).toLocaleString("en-US")}</strong>
                </div>
                <div className="settings-item">
          ...
          {activeTab === "Aging" ? (
            <>
              <div className="settings-grid settings-stats-grid">
                ...
                <div className="settings-item">
                  <span className="settings-label">Reserved Qty</span>
                  <strong>{agingRows.reduce((sum, row) => sum + row.reserved_qty, 0).toLocaleString("en-US")}</strong>
                </div>
                <div className="settings-item">
          ...
          {activeTab === "Forecast" ? (
            <>
              <div className="settings-grid settings-stats-grid">
                ...
                <div className="settings-item">
                  <span className="settings-label">Reserved Qty</span>
                  <strong>{forecastRows.reduce((sum, row) => sum + row.reserved_qty, 0).toLocaleString("en-US")}</strong>
                </div>
                <div className="settings-item">
