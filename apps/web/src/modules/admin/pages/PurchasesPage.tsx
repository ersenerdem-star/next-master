...
export function PurchasesPage({
  ...
  const [downloadingPurchaseOrderPdf, setDownloadingPurchaseOrderPdf] = useState(false);
  const [printingBill, setPrintingBill] = useState(false);
  const [downloadingBillPdf, setDownloadingBillPdf] = useState(false);
  ...
  function handlePrintPurchaseOrder(row: LocalPurchaseOrder) {
    ...
    try {
      ...
      openBusinessDocumentPreview(buildPurchaseOrderHtml(row));
  ...
  function handleDownloadPurchaseOrderPdf(row: LocalPurchaseOrder) {
    setDownloadingPurchaseOrderPdf(true);
    try {
      actionFeedback.begin(`Preparing PDF download for purchase order ${row.id}...`);
      void downloadPdfFromHtml({
        html: buildPurchaseOrderHtml(row),
        filename: `${sanitizeFileName(row.id || "purchase-order")}.pdf`,
      })
        .then(() => actionFeedback.succeed(`PDF downloaded for ${row.id}.`))
        .catch((caught) => {
          actionFeedback.fail(caught instanceof Error ? caught.message : "Purchase order PDF download failed");
        })
        .finally(() => setDownloadingPurchaseOrderPdf(false));
    } catch (caught) {
      actionFeedback.fail(caught instanceof Error ? caught.message : "Purchase order PDF download failed");
      setDownloadingPurchaseOrderPdf(false);
    }
  }
  ...
  function handlePrintBill(row: LocalBill) {
    ...
    try {
      ...
      openBusinessDocumentPreview(buildBillHtml(row));
  ...
  function handleDownloadBillPdf(row: LocalBill) {
    setDownloadingBillPdf(true);
    try {
      actionFeedback.begin(`Preparing PDF download for bill ${row.id}...`);
      void downloadPdfFromHtml({
        html: buildBillHtml(row),
        filename: `${sanitizeFileName(row.id || "bill")}.pdf`,
      })
        .then(() => actionFeedback.succeed(`PDF downloaded for ${row.id}.`))
        .catch((caught) => {
          actionFeedback.fail(caught instanceof Error ? caught.message : "Bill PDF download failed");
        })
        .finally(() => setDownloadingBillPdf(false));
    } catch (caught) {
      actionFeedback.fail(caught instanceof Error ? caught.message : "Bill PDF download failed");
      setDownloadingBillPdf(false);
    }
  }
          ...
          {purchaseOrderDraft && purchaseOrdersView === "detail" ? (
            <div className="invoice-editor-block">
                ...
                <div className="toolbar toolbar--wrap">
                  ...
                  <Button variant="secondary" onClick={() => handleDownloadPurchaseOrderPdf(purchaseOrderDraft)} busy={downloadingPurchaseOrderPdf} busyLabel="Preparing...">
                    Download PDF
                  </Button>
            ...
            <div className="invoice-editor-block">
              <div className="invoice-edit-shell">
                ...
                <div className="toolbar toolbar--wrap">
                  ...
                  <Button variant="secondary" onClick={() => handleDownloadBillPdf(billDraft)} busy={downloadingBillPdf} busyLabel="Preparing...">
                    Download PDF
                  </Button>
