...
export function SalesPage({
    ...
    async function run() {
      try {
        if (activeTab === "Invoices") {
          const [invoiceRows, salesOrderRows, warehouseRows] = await Promise.all([
            fetchInvoiceSummaries(),
            fetchSalesOrderSummaries(),
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        if (!cancelled) {
          setInvoiceDraft(cloneInvoice(applyDefaultInvoiceWarehouse(detail)));
        }
      } catch {
        if (!cancelled) {
          setInvoiceDraft(null);
  ...
  function buildInvoiceHtml(row: LocalInvoice) {
    ...
    return buildBusinessDocumentHtml({
      ...
      meta: [
        ...
        { label: "Warehouse", value: row.warehouse_name || row.warehouse_code || "-" },
      ],
  ...
  function handleDownloadInvoicePdf(row: LocalInvoice) {
    try {
      actionFeedback.begin(`Preparing PDF download for ${row.id}...`);
      void downloadPdfFromHtml({
        html: buildInvoiceHtml(row),
        filename: `${sanitizeFileName(row.id || "invoice")}.pdf`,
      })
        .then(() => actionFeedback.succeed(`PDF downloaded for ${row.id}.`))
        .catch((caught) => {
          actionFeedback.fail(caught instanceof Error ? caught.message : "Invoice PDF download failed");
        });
    } catch (caught) {
      actionFeedback.fail(caught instanceof Error ? caught.message : "Invoice PDF download failed");
    }
  ...
  function handleExportInvoiceExcel(row: LocalInvoice) {
    try {
      const currency = row.currency || "EUR";
      const rows: Array<Array<string | number | null | undefined>> = [
        ["Invoice", row.id],
        ["Customer", row.customer_name || ""],
        ["Seller Company", row.seller_company || ""],
        ["Sales Order", row.sales_order_no || ""],
        ["Warehouse", row.warehouse_name || row.warehouse_code || ""],
        ["Status", row.status || ""],
        ["Currency", currency],
        ["Invoice Date", row.quote_date || ""],
        ["Due Date", row.due_date || ""],
        ["Payment Terms", row.payment_terms || ""],
        ["Delivery Term", row.delivery_term || ""],
        ["Contract Nr", row.contract_nr || ""],
        ["Packing Details", row.packing_details || ""],
        [],
        ["Code", "Brand", "Description", "Qty", "OEM", "HS Code", "Origin", "Weight", `Unit Price ${currency}`, `Line Total ${currency}`, "Notes"],
        ...row.lines.map((line) => [
          ...
          Number(line.sell_price || 0),
          Number(line.sales_total || 0),
          line.notes || "",
        ]),
        [],
      ...
      const blob = buildXlsxBlob((row.id || "invoice").slice(0, 31), rows, [3, 7, 8, 9]);
      downloadBlob(`${sanitizeFileName(row.id || "invoice")}.xlsx`, blob);
      actionFeedback.succeed(`Excel exported for ${row.id}.`);
    } catch (caught) {
      actionFeedback.fail(caught instanceof Error ? caught.message : "Invoice Excel export failed");
    }
  }
  ...
  const invoiceWarehouseOptions = useMemo(
    () =>
      warehouses
        .filter((warehouse) => warehouse.is_active && warehouse.fulfillment_model !== "dropship")
        .map((warehouse) => ({
          value: warehouse.id,
  ...
  const selectedInvoiceWarehouse = useMemo(
    () => warehouses.find((warehouse) => warehouse.id === invoiceDraft?.warehouse_id) || null,
    [warehouses, invoiceDraft?.warehouse_id],
  );
  ...
  function handleInvoiceWarehouseChange(warehouseId: string) {
    const nextWarehouse = warehouses.find((warehouse) => warehouse.id === warehouseId) || null;
  ...
  function applyDefaultInvoiceWarehouse(invoice: LocalInvoice) {
    if (invoice.warehouse_id) return invoice;
    const defaultWarehouse = warehouses.find((warehouse) => warehouse.is_active && warehouse.fulfillment_model !== "dropship") || warehouses[0] || null;
    if (!defaultWarehouse) return invoice;
    return {
      ...invoice,
  ...
  useEffect(() => {
    if (!invoiceDraft || invoiceDraft.warehouse_id || !invoiceWarehouseOptions.length) return;
    const firstWarehouse = warehouses.find((warehouse) => warehouse.id === invoiceWarehouseOptions[0]?.value) || null;
    ...
    setInvoiceDraft((current) =>
      current
        ? {
            ...
            warehouse_code: firstWarehouse.warehouse_code,
            warehouse_name: firstWarehouse.warehouse_name,
          }
        : current,
    );
  }, [invoiceDraft, invoiceWarehouseOptions, warehouses]);
  ...
  async function handleResyncInvoiceFromCatalog() {
    ...
    try {
      ...
      const draftWithWarehouse = applyDefaultInvoiceWarehouse(invoiceDraft);
      const salesOrderCustomerType = salesOrders.find((row) => row.id === draftWithWarehouse.sales_order_id)?.customer_type || "A";
      const nextLines = await resyncInvoiceLinesFromCatalog(draftWithWarehouse.lines, {
      ...
      setInvoiceDraft(cloneInvoice(applyDefaultInvoiceWarehouse(saved)));
  ...
  async function saveInvoiceDraft() {
    if (!invoiceDraft) return;
    const draftWithWarehouse = applyDefaultInvoiceWarehouse(invoiceDraft);
    if (draftWithWarehouse.status !== "draft" && !draftWithWarehouse.warehouse_id) {
      actionFeedback.fail("Select a warehouse before confirming the invoice.");
      return;
    }
    const previousId = selectedInvoiceId;
    const payload: LocalInvoice = {
      ...recomputeInvoiceTotals(draftWithWarehouse),
      discount_amount: Number(draftWithWarehouse.discount_amount || 0),
      shipping_cost: Number(draftWithWarehouse.shipping_cost || 0),
  ...
  async function handleCreateInvoicesFromSelection() {
    ...
    try {
      ...
      if (created[0]) {
        ...
        setInvoiceDraft(cloneInvoice(applyDefaultInvoiceWarehouse(created[0])));
      }
  ...
  async function handleMergeInvoicesFromSelection() {
    ...
    try {
      ...
      setInvoiceDraft(cloneInvoice(applyDefaultInvoiceWarehouse(merged)));
  ...
  const invoiceColumns = [
    ...
    { key: "salesOrder", header: "Sales Order", render: (row: LocalInvoice) => row.sales_order_no, sortValue: (row: LocalInvoice) => row.sales_order_no },
    {
      key: "warehouse",
      header: "Warehouse",
      render: (row: LocalInvoice) => row.warehouse_name || row.warehouse_code || "-",
      sortValue: (row: LocalInvoice) => row.warehouse_name || row.warehouse_code || "",
    },
    {
      key: "brands",
      header: "Brand",
              ...
              <div className="invoice-edit-shell">
                ...
                <div className="invoice-meta-grid">
                  ...
                  <Select
                    ...
                    options={[{ value: "", label: "Select warehouse" }, ...invoiceWarehouseOptions]}
                    onChange={handleInvoiceWarehouseChange}
                  />
                  <Select
                    label="Terms"
                ...
                <div className="info-text" style={{ marginBottom: 12 }}>
                  {selectedInvoiceWarehouse ? `Final stock deduction will post from ${selectedInvoiceWarehouse.warehouse_name}.` : "Select a warehouse before confirming the invoice."}
                </div>
                ...
                <div className="toolbar toolbar--wrap">
                  ...
                  <Button variant="secondary" onClick={() => handleExportInvoiceExcel(invoiceDraft)}>
                    Export Excel
                  </Button>
                  <Button variant="secondary" onClick={() => handlePrintInvoice(invoiceDraft)}>
                    Preview PDF
                  </Button>
                  <Button variant="secondary" onClick={() => handleDownloadInvoicePdf(invoiceDraft)}>
                    Download PDF
                  </Button>
