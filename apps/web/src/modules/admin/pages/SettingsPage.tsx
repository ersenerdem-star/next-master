...
type NewUserDraft = {
  email: string;
  fullName: string;
  role: "admin" | "warehouse" | "sales" | "viewer";
  isActive: boolean;
};
...
type EditUserDraft = {
  ...
  email: string;
  fullName: string;
  role: "superadmin" | "admin" | "warehouse" | "sales" | "viewer";
  isActive: boolean;
};

type SettingsPageProps = {
  onLogout?: () => void | Promise<void>;
  onForceSignOutAll?: () => void | Promise<void>;
...
export function SettingsPage({ onLogout, onForceSignOutAll, initialTab = "session", onOpenRelatedRecord }: SettingsPageProps) {
  ...
  function openUserEditor(row: OrgUser) {
    setEditUserDraft({
      ...
      role: (row.role === "superadmin" || row.role === "admin" || row.role === "warehouse" || row.role === "sales" || row.role === "viewer" ? row.role : "sales") as EditUserDraft["role"],
  ...
  const staffCount = users.filter((user) => user.role === "warehouse" || user.role === "sales" || user.role === "viewer").length;
  ...
  function setPortalBrandScopeMode(mode: "all" | "selected") {
    setPortalDraft((current) => {
      if (mode === "all") {
        return {
          ...current,
      ...
      if (current.allowed_brand_ids.length) {
        return current;
      }

      const firstBrandId = brandOptions[0]?.id ? [brandOptions[0].id] : [];
      return {
        ...current,
        allowed_brand_ids: firstBrandId,
      };
    });
  }
  ...
  const portalBrandScopeMode = portalDraft.allowed_brand_ids.length ? "selected" : "all";
    ...
    <div className="page-stack">
      {activeTab === "session" ? <SectionCard title="Session Settings">
        ...
        <div className="toolbar toolbar--wrap">
          ...
          {isSuperadminRole(state.role) && onForceSignOutAll ? (
            <Button
              variant="secondary"
              className="danger-button"
              busy={loggingOut}
              busyLabel="Signing out all users..."
              onClick={async () => {
                try {
                  setLoggingOut(true);
                  actionFeedback.begin("Signing out all users...");
                  await onForceSignOutAll?.();
                  actionFeedback.succeed("All users signed out.");
                } catch (caught) {
                  const message = caught instanceof Error ? caught.message : "Force sign-out failed";
                  actionFeedback.fail(message);
                } finally {
                  setLoggingOut(false);
                }
        ...
        <SectionCard title="Users">
          <div className="stats-grid stats-grid--compact">
            <StatCard
              ...
              subtext="Active organization identities under Next Master control"
              tone="neutral"
            />
            <StatCard
              label="Superadmins"
              value={superadminCount.toLocaleString("en-US")}
              subtext="Users allowed to govern system, warehouse, and company records"
              tone="success"
            />
            <StatCard
              label="Admins"
              value={adminCount.toLocaleString("en-US")}
              subtext="Operational leaders with warehouse and purchasing authority"
              tone="neutral"
            />
            <StatCard
              label="Staff Accounts"
              value={staffCount.toLocaleString("en-US")}
              subtext="Warehouse, sales, and viewer accounts routed into controlled work areas"
              tone={staffCount ? "success" : "warning"}
            />
          </div>
          {editUserDraft ? (
      ...
      {activeTab === "portals" ? <SectionCard title="Customer & Vendor Portal Access">
        ...
        <div className="field portal-scope-panel">
          <div className="portal-scope-panel__header">
            <Select
              label="Brand Scope"
              value={portalBrandScopeMode}
              options={[
                { value: "all", label: "All Brands" },
                { value: "selected", label: "Brand Select" },
              ]}
              onChange={(value) => setPortalBrandScopeMode(value === "all" ? "all" : "selected")}
              fieldClassName="portal-scope-panel__mode"
            />
            <div className="portal-scope-panel__summary">
              <span>{portalBrandScopeMode === "all" ? "All brands included" : `${portalDraft.allowed_brand_ids.length} brand selected`}</span>
              <Button variant="secondary" className="button--compact" onClick={() => updatePortalField("allowed_brand_ids", [])}>
                Clear
              </Button>
            </div>
          </div>
          {portalBrandScopeMode === "selected" ? (
            <div className="portal-scope-panel__brands">
              {brandOptions.map((brand) => (
                <label key={brand.id} className="field checkbox-field portal-scope-panel__brand">
                  <input
                    type="checkbox"
            ...
            <span className="field__help">
              All brands are included in portal search, price list, and order flow.
            </span>
          )}
          <span className="field__help">
            Choose All Brands for unrestricted portal access, or Brand Select to limit the portal to specific brands only.
          </span>
        </div>
        <div className="toolbar toolbar--wrap">
          <Button
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                const result = await fetchAdminDiagnostics(testEmail.trim() || undefined);
                setDiagnostics(result);
                const message = result.emailTest?.attempted
                  ? `Diagnostics loaded. ${result.emailTest.detail}`
                  : "Diagnostics loaded.";
        ...
        {diagnostics ? (
          <>
            <div className="settings-grid settings-stats-grid">
              ...
              <div className="settings-item">
                <span className="settings-label">Deploy Context</span>
                <strong>{diagnostics.runtime.deployContext || "-"}</strong>
              </div>
              <div className="settings-item">
              ...
              <div className="settings-item">
                <span className="settings-label">Generated</span>
                <strong>{diagnostics.runtime.generatedAt || "-"}</strong>
              </div>
            </div>
            <div className="settings-grid settings-stats-grid">
              {[
                ...
                ["Portal Secret", diagnostics.env.portalSessionSecret],
                ...
                ["Portal Check", diagnostics.checks.portal.ok],
                ["Warehouse Check", diagnostics.checks.warehouse.ok],
                ["Catalog Check", diagnostics.checks.catalog.ok],
              ].map(([label, ok]) => (
            ...
            <div className="settings-grid settings-stats-grid">
              {[
                ["Brands", diagnostics.counts.brands],
                ["Catalog Rows", diagnostics.counts.catalogProducts],
                ["Supplier Prices", diagnostics.counts.activeSupplierPrices],
                ["Portal Active", `${diagnostics.counts.portalInvitesActive} / ${diagnostics.counts.portalInvitesTotal}`],
                ["Portal Disabled", diagnostics.counts.portalInvitesDisabled],
                ["Queued Emails", diagnostics.counts.queuedEmails],
                ["Failed Emails", diagnostics.counts.failedEmails],
                ["Warehouses", diagnostics.counts.activeWarehouses],
              ].map(([label, value]) => (
                <div key={String(label)} className="settings-item">
                  <span className="settings-label">{label}</span>
                  <strong>{value}</strong>
            ...
            <div className="meta-row">
              <span>{diagnostics.checks.catalog.detail}</span>
              <span>{diagnostics.checks.portal.detail}</span>
            </div>
            <div className="meta-row">
              <span>{diagnostics.checks.warehouse.detail}</span>
              <span>{diagnostics.checks.email.detail}</span>
            </div>
            {diagnostics.warnings.length ? (
              <div className="warning-list">
                {diagnostics.warnings.map((warning) => (
                  <div key={warning} className="warning-list__item">{warning}</div>
                ))}
              </div>
            ) : (
              <div className="success-text">No active diagnostics warnings.</div>
            )}
            {diagnostics.emailTest?.attempted ? (
              <div className={diagnostics.emailTest.ok ? "success-text" : "warning-text"}>
                {diagnostics.emailTest.detail}
              </div>
            ) : null}
          </>
        ) : (
          <div className="meta-row">
            ...
            <span>Run diagnostics to validate production runtime, catalog footprint, portal access, warehouse scope, and outbound email flow.</span>
          </div>
        )}
        <div className="settings-grid">
