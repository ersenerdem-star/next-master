...
begin
  ...
  if normalized_code_generated = 'ALWAYS' then
    ...
    where b.id = cp.brand_id
      and public.normalize_catalog_brand_key(b.name) in ('KNORR', 'KNORRBREMSE')
...
create or replace function public.normalize_knorr_bremse_order_lines(input_lines jsonb)
returns jsonb
language plpgsql
immutable
set search_path = public
...
begin
  ...
  loop
    next_item := item;
    if public.normalize_catalog_brand_key(next_item->>'brand') in ('KNORR', 'KNORRBREMSE') then
      if next_item ? 'product_code' then
        next_item := jsonb_set(next_item, '{product_code}', to_jsonb(public.normalize_catalog_display_code_for_brand(next_item->>'product_code', 'Knorr-Bremse')), true);
      end if;
      if next_item ? 'old_code' then
        next_item := jsonb_set(next_item, '{old_code}', to_jsonb(public.normalize_catalog_display_code_for_brand(next_item->>'old_code', 'Knorr-Bremse')), true);
      end if;
      if next_item ? 'requestedCode' then
        next_item := jsonb_set(next_item, '{requestedCode}', to_jsonb(public.normalize_catalog_display_code_for_brand(next_item->>'requestedCode', 'Knorr-Bremse')), true);
      end if;
      if next_item ? 'resolvedCode' then
        next_item := jsonb_set(next_item, '{resolvedCode}', to_jsonb(public.normalize_catalog_display_code_for_brand(next_item->>'resolvedCode', 'Knorr-Bremse')), true);
      end if;
    end if;
...
set lines = public.normalize_knorr_bremse_order_lines(lines),
    updated_at = now()
where jsonb_typeof(lines) = 'array'
  and lines is distinct from public.normalize_knorr_bremse_order_lines(lines);
