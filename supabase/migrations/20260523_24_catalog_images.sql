...
where trim(coalesce(image_url, '')) = ''
   or lower(coalesce(image_url, '')) like '%/no-image.%';
...
begin
  ...
  select
    ...
    weight_kg,
    case
      when lower(trim(coalesce(image_url, ''))) like '%/no-image.%' then null
      else nullif(trim(coalesce(image_url, '')), '')
    end,
    case
  ...
  where
    ...
    or (excluded.image_url is not null and public.catalog_products.image_url is distinct from excluded.image_url)
