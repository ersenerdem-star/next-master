...
alter table public.catalog_products
  add column if not exists normalized_ean text generated always as (public.normalize_part_code(coalesce(ean, ''))) stored;

create index if not exists idx_catalog_products_org_normalized_ean
  on public.catalog_products (organization_id, normalized_ean)
  where normalized_ean <> '';

create index if not exists idx_catalog_products_org_brand_normalized_ean
  on public.catalog_products (organization_id, brand_id, normalized_ean)
  where normalized_ean <> '';
      ...
      and (
        ...
        or (
          ...
          and (
            ...
            or coalesce(cp.ean, '') ilike '%' || f.raw_search || '%'
