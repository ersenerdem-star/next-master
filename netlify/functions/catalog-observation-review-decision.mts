...
export async function handleCatalogObservationReviewDecisionRequest(
  req: Request,
  _context: Context,
  deps = {
  ...
  try {
    ...
    const command = validateDecisionCommand(await parseJsonCommandBody(req));
    const db = deps.createCatalogObservationDecisionCommandDb({ supabaseUrl, supabaseAnonKey, accessToken });
    const result = await db.recordDecision(command);
    return json(serializeDecisionResult(result, { action: "record_decision" }));
  } catch (error) {
...
export default async (req: Request, context: Context) => handleCatalogObservationReviewDecisionRequest(req, context);

export const config: Config = {
  path: "/api/catalog/observation-review/decision",
  method: "POST",
};
