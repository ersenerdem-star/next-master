...
export async function syncBrandCatalogFromSkfAutomotive(input: {
  ...
  const existingRows = await fetchCatalogRows(input.supabaseUrl, headers, target, supportsEanColumn, supportsMarketSegmentColumn);
  ...
  for (const row of existingRows) {
    ...
    if (!scoped) continue;
    if (refreshExisting || shouldProcessRow(row, supportsEanColumn, supportsMarketSegmentColumn)) {
  ...
  await runPool([...workMap.values()], concurrency, async (item) => {
    try {
      ...
      const changed = !item.existing || hasCatalogDelta(item.existing, merged, supportsEanColumn, supportsMarketSegmentColumn);
  ...
  const incompleteExistingRows = existingRows.filter((row) => shouldProcessRow(row, supportsEanColumn, supportsMarketSegmentColumn)).length;
...
async function fetchCatalogRows(
  ...
  target: SyncBrandTarget,
  supportsEanColumn: boolean,
  supportsMarketSegmentColumn: boolean,
) {
...
async function fetchSkfProductPage(productCode: string, requestTimeoutMs: number) {
...
async function resolveSkfItem(current: CatalogRow | null, searchItem: SkfSearchItem | null, requestTimeoutMs: number): Promise<SkfResolvedItem> {
  ...
  return {
    product_code: productCode,
    description: extractSkfDescription(detail, searchItem, current, officialPage.description, productCode),
    ...
    origin: normalizeCatalogOrigin(extractArticleInfoValue(articleInformation, [/country of origin/i, /^origin$/i, /made in/i]) || "") || normalizeOriginValue(extractArticleInfoValue(articleInformation, [/country of origin/i, /^origin$/i, /made in/i]) || ""),
...
function extractSkfDescription(
  detail: SkfDetailDocument | null,
  searchItem: SkfSearchItem | null,
  current: CatalogRow | null,
  officialPageDescription: string,
  productCode: string,
) {
  return pickCatalogDescription(
    [
      officialPageDescription,
      detail?.fb1 || "",
      detail?.fb2 || "",
      detail?.type || "",
      searchItem?.type || "",
...
function extractSkfMarketSegment(detail: SkfDetailDocument | null, fallback: string) {
...
function parseSkfOfficialProductPage(html: string, productCode: string) {
  ...
  try {
    const payload = JSON.parse(productSchemaMatch[1]) as Record<string, unknown>;
    const description = normalizeCatalogDescription(String(payload.description || "").trim());
    const possibleEan = extractPossibleSkfPageEan(html, productCode);
    return {
      description,
      ean: possibleEan,
    };
...
function extractPossibleSkfPageEan(html: string, productCode: string) {
...
function hasCatalogDelta(current: CatalogRow, next: CatalogRow, supportsEanColumn: boolean, supportsMarketSegmentColumn: boolean) {
  return (
...
function shouldProcessRow(row: CatalogRow, supportsEanColumn: boolean, supportsMarketSegmentColumn: boolean) {
  return (
