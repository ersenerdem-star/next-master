...
export async function syncBrandCatalogFromBoschAftermarket(input: {
  ...
  for (const row of existingRows) {
    ...
    if (refreshExisting ? isScopedExistingRow : isScopedExistingRow && hasOfficialSearchMatch && shouldProcessRow(row, supportsEanColumn)) {
  ...
  await runPool(workItems, concurrency, async (item) => {
    try {
      ...
      const changed = !item.existing || hasCatalogDelta(item.existing, merged, supportsEanColumn);
...
async function fetchBoschCatalogRows(supabaseUrl: string, headers: Record<string, string>, target: any) {
  ...
  while (true) {
    ...
    if (!response.ok) {
      if (supportsEanColumn && /column .*ean/i.test(text)) {
        supportsEanColumn = false;
        continue;
      }
      throw new Error(`catalog_products fetch failed for Bosch: ${response.status} ${text}`);
    }
  ...
  return {
    ...
    items: items
      .map((item: any) => ({
        ...
        ean: normalizeCatalogEan(String(item.ean || item.gtin || item.barcode || "")),
...
function mergeCatalogRow(target: any, existing: any, searchItem: any, detail: any, vehicles: string[]) {
  ...
  return {
    ...
    ean: extractBoschEan(detail, searchItem, existing),
  ...
  return (
    pickCatalogDescription(
      [
        ...
        extractBoschSpecificationValue(detail, ["Tanımlama", "Identification"]),
        cleanText(searchItem?.description || ""),
        cleanText(existing?.description || ""),
        cleanText(detail?.longDescription || ""),
        cleanText(detail?.shortDescription || ""),
      ],
      referenceCode,
    ) || ""
  );
...
function extractBoschEan(detail: any, searchItem: any, existing: any) {
  const candidates = [
    extractSpecificationValue(detail?.specificationTabData, ["EAN", "GTIN", "Barcode"]),
    extractSpecificationValue(detail?.generalTabData, ["EAN", "GTIN", "Barcode"]),
    extractSpecificationValue(detail?.technicalTabData, ["EAN", "GTIN", "Barcode"]),
    extractSpecificationValue(detail?.logisticsTabData, ["EAN", "GTIN", "Barcode"]),
    normalizeCatalogEan(String(detail?.ean || detail?.gtin || detail?.barcode || "")),
    normalizeCatalogEan(String(searchItem?.ean || searchItem?.gtin || searchItem?.barcode || "")),
    normalizeCatalogEan(String(existing?.ean || "")),
  ];
...
function shouldProcessRow(row: any, supportsEanColumn: boolean) {
  ...
  if (isCatalogPlaceholderDescription(row.description, row.normalized_code || row.product_code)) return true;
  if (supportsEanColumn && !normalizeCatalogEan(row.ean || "")) return true;
  if (normalizeLifecycleStatus(row.lifecycle_status) === "discontinued" && !normalizeTextValue(row.lifecycle_note)) return true;
  return false;
}

function hasCatalogDelta(existing: any, next: any, supportsEanColumn: boolean) {
  return (
    ...
    (supportsEanColumn && normalizeCatalogEan(existing.ean || "") !== normalizeCatalogEan(next.ean || "")) ||
...
function formatBoschDisplayCode(value: unknown) {
  return normalizeCatalogDisplayCode(String(value || ""), "Bosch");
}
