...
async function fetchCatalogRows(supabaseUrl: string, headers: Record<string, string>, target: any, supportsEanColumn: boolean) {
  const results = [];
  const pageLimit = 1000;
  let offset = 0;
  let detectedSupportsEanColumn = supportsEanColumn;
  while (true) {
    const selectColumns = [
      "organization_id",
      "brand_id",
    ...
    if (!response.ok) {
      if (detectedSupportsEanColumn && /column .*ean/i.test(text)) {
        detectedSupportsEanColumn = false;
        continue;
      }
...
function extractEan(details: any, specEntries: Array<{ label: string; values: string[] }>) {
...
function formatOfficialDescription(target: any, value: unknown) {
  ...
  if (isCatalogPlaceholderDescription(stripped, "")) return "";
