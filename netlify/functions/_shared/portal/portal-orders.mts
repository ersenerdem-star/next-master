import { buildRestUrl, getJson, sendJson, serviceRoleHeaders } from "../core/http.mts";
import { canonicalizeInternalBrandName, normalizeBrandKey } from "../catalog/brand-standardization.mts";
import { normalizeCatalogMarketSegment } from "../catalog/catalog-segments.mts";
import { buildPortalSnapshot } from "./portal-access.mts";
import { normalizeLifecycleStatus, sanitizeCatalogOemNumbers } from "../catalog/catalog-standardization.mts";
import {
  getDisplayPriceListType,
...
const PORTAL_PRICE_LIST_SUPPLIER_PAGE_SIZE = 1000;
const PORTAL_PRICE_LIST_ITEM_PAGE_SIZE = 5000;
const PORTAL_SUPPLIER_LOOKUP_CODE_CHUNK_SIZE = 120;
const PORTAL_PREPARE_BATCH_SIZE = 15;
const PORTAL_PREPARE_QUERY_CHUNK_SIZE = 40;
...
const portalCatalogSearchCache = new Map<string, PortalLookupCacheEntry<PortalCatalogSearchResponse>>();
...
function portalCatalogPreviewKey(brand: string, normalizedCode: string) {
  return `${brand.trim().toLowerCase()}::${normalizedCode}`;
}

function buildPortalSearchCacheKey(invite: PortalInviteRow, query: string, brand: string) {
  return [
    invite.organization_id,
    invite.id,
    String(invite.customer_id || ""),
    String(invite.vendor_id || ""),
    String(brand || "").trim().toLowerCase(),
    String(query || "").trim().toLowerCase(),
    normalizePartCode(query),
  ].join("::");
}

function shouldLoadPortalRecommendations(search: string, baseItems: SearchCatalogBaseItem[]) {
  return shouldStrictlyFilterCodeSearch(search) && baseItems.length > 0 && baseItems.length <= 3;
}
...
function buildPortalExactSearchVariants(search: string) {
  return [...new Set([normalizePartCode(search), normalizeOriginalNumberSearch(search), ...buildOriginalNumberVariants(search)])]
    .map((variant) => String(variant || "").trim())
    .filter((variant) => variant.length >= 6)
    .slice(0, 8);
}

async function fetchPortalExactSearchRows(
  supabaseUrl: string,
  ...
  const canRunPrefixFallback = Boolean(selectedBrandId || allowedBrandIds.length === 1);
  const productCodeClauses = canRunPrefixFallback
    ? variants
    .filter((variant) => /[A-Z]/.test(variant))
    .slice(0, 6)
    .map((variant) => `product_code.ilike.${variant}*`)
    : [];
  const [exactCodeRows, exactOemRows] = await Promise.all([
...
function filterCatalogRowsBySearchRelevance(
  ...
  return filtered.length ? filtered : exactRows;
}

type PortalSearchMode = "strict" | "loose";
const PORTAL_CODE_SEARCH_EXPANSION_THRESHOLD = 4;
...
function isPortalTimeoutLike(error: unknown) {
...
function buildPortalTimeoutRetryOr(search: string) {
  const variants = [...new Set([normalizePartCode(search), normalizeOriginalNumberSearch(search), ...buildOriginalNumberVariants(search)])]
    .map((variant) => String(variant || "").trim())
    .filter((variant) => variant.length >= 6)
    .slice(0, 8);
  const clauses = new Set<string>();
...
async function buildPortalCatalogPreviewMap(
  supabaseUrl: string,
  serviceRoleKey: string,
  invite: PortalInviteRow,
  ...
  >();
  if (!baseItems.length) return previewByCode;
  ...
  if (context.customerType === "C") {
    ...
    const marginPercent = getSupplierFallbackCustomerType(context.customerType) === "B" ? context.effectiveMarginB : context.effectiveMarginA;
...
function mapPortalCatalogItems(
  baseItems: SearchCatalogBaseItem[],
  previewByCode: Map<
    string,
    {
  ...
  return baseItems.map((item) => {
    const preview = previewByCode.get(portalCatalogPreviewKey(item.brand, item.normalized_code));
    return {
      code: item.code,
      brand: item.brand,
...
async function fetchAll<T>(
  ...
  const rows: T[] = [];
  const baseOffset = parsePositiveInteger(params.offset) ?? 0;
  let pageOffset = baseOffset;

  while (rows.length < requestedLimit) {
    const pageLimit = Math.min(POSTGREST_PAGE_CAP, requestedLimit - rows.length);
    const page = await getJson<Array<T>>(
      buildRestUrl(supabaseUrl, table, {
        ...params,
...
function brandLookupVariants(value: string) {
  const raw = String(value || "").trim();
  const canonical = canonicalizeInternalBrandName(raw);
  const values = [raw, canonical]
    .flatMap((item) => [item, normalizeBrandKey(item)])
    .map((item) => String(item || "").trim())
    .filter(Boolean);
  return [...new Set(values.map((item) => item.toLowerCase()))];
}

function resolveBrandIdsFromMap(brandMap: { byId: Map<string, string>; byName: Map<string, string> }, requestedBrand: string) {
  const requestedKeys = new Set(brandLookupVariants(requestedBrand));
  if (!requestedKeys.size) return [];
  const matches = new Set<string>();
  for (const [id, name] of brandMap.byId.entries()) {
    if (brandLookupVariants(name).some((key) => requestedKeys.has(key))) {
      matches.add(id);
    }
  }
  for (const key of requestedKeys) {
    const direct = brandMap.byName.get(key) || brandMap.byName.get(key.toLowerCase()) || brandMap.byName.get(key.toUpperCase());
...
async function resolvePortalCustomer(
  ...
  const priceListType = normalizeCustomerPricingType(String(customer.price_list_type || getEmbeddedCustomerPriceListType(customerMeta) || "A"));
...
async function resolveBrandMap(supabaseUrl: string, serviceRoleKey: string, organizationId: string) {
  ...
  for (const row of brands) {
    ...
    if (!id || !name) continue;
    byId.set(id, name);
    for (const key of brandLookupVariants(name)) {
      byName.set(key, id);
    }
  }
...
export async function searchPortalCatalog(
  ...
  if (search && strictCodeSearch && normalizedSearch.length >= 3 && rows.length < 24) {
  ...
  if (search && shouldRunLooseOriginalNumberSearch(search) && rows.length < PORTAL_CODE_SEARCH_EXPANSION_THRESHOLD) {
  ...
  if (!baseItems.length) {
    const emptyResult = { items: [], recommendations: [] };
    console.info(
      JSON.stringify({
        ...
        brandScoped: Boolean(selectedBrandId),
        items: 0,
        recommendations: 0,
        ms: Date.now() - startedAt,
      }),
    );
    portalCatalogSearchCache.set(searchCacheKey, {
      value: emptyResult,
      expiresAt: Date.now() + PORTAL_SEARCH_RESULT_CACHE_TTL_MS,
    });
    return emptyResult;
  }
  ...
  console.info(
    JSON.stringify({
      ...
      replacementMatches: replacementRowsByKey.size,
      ms: Date.now() - startedAt,
    }),
  );
  portalCatalogSearchCache.set(searchCacheKey, {
...
function mergeInputRows(rows: PortalOrderInputRow[]) {
  ...
  for (const row of rows) {
    ...
    const key = `${brand.toLowerCase()}::${marketSegment}::${normalizePartCode(code)}`;
    const current = grouped.get(key);
    if (current) {
      current.qty += qty;
    } else {
      grouped.set(key, { code, brand, qty, market_segment: marketSegment || null });
    }
  }
...
async function fetchPortalBrandCPriceEntryMap(
  supabaseUrl: string,
  serviceRoleKey: string,
  organizationId: string,
  brandId: string,
  ...
  while (true) {
    const items = await fetchAll<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "customer_price_list_items", {
      select: "price_list_id,normalized_code,sell_price,updated_at",
      organization_id: `eq.${organizationId}`,
      price_list_id: `in.(${cPriceListIds.join(",")})`,
      brand_id: `eq.${brandId}`,
      order: "normalized_code.asc",
    ...
    for (const row of items) {
      ...
      if (currentPriority != null && currentPriority === priority) {
        const currentDate = map.get(key)?.price_date || "";
        const nextDate = row.updated_at == null ? "" : String(row.updated_at).slice(0, 10);
        if (currentDate && nextDate && currentDate >= nextDate) continue;
      }
      map.set(key, {
        sell_price: Number(row.sell_price || 0),
        price_date: row.updated_at == null ? null : String(row.updated_at).slice(0, 10),
...
function portalCPriceEntryNormalizedCode(key: string) {
...
async function fetchPortalCatalogBrandRows(
  ...
  while (true) {
    const page = await fetchAll<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "catalog_products", {
      select: "product_code,description,oem_no,hs_code,origin,market_segment,weight_kg,lifecycle_status,lifecycle_note",
      organization_id: `eq.${organizationId}`,
      brand_id: `eq.${brandId}`,
      ...(lastProductCode ? { product_code: `gt.${lastProductCode}` } : {}),
      order: "product_code.asc",
    ...
    lastProductCode = String(page[page.length - 1]?.product_code || "").trim();
    if (!lastProductCode) break;
  }
...
async function fetchPortalBestSupplierOptionMap(
  ...
  >();
  const uniqueCodes = [...new Set(normalizedCodes.map((code) => String(code || "").trim()).filter(Boolean))];
  if (!uniqueCodes.length) return bestByCode;

  for (const codeChunk of chunkValues(uniqueCodes, PORTAL_SUPPLIER_LOOKUP_CODE_CHUNK_SIZE)) {
    const supplierRows = await fetchAll<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "supplier_prices", {
    ...
    for (const row of supplierRows) {
      ...
      const normalizedCode = String(row.normalized_code || "");
      const buyPrice = row.buy_price == null ? null : Number(row.buy_price);
      if (buyPrice == null || !Number.isFinite(buyPrice)) continue;
      const current = bestByCode.get(normalizedCode);
...
async function fetchPortalBestSupplierPreviewMap(
  ...
  for (const brandChunk of chunkValues(brandIds, 50)) {
    for (const codeChunk of chunkValues(normalizedCodes, PORTAL_SUPPLIER_LOOKUP_CODE_CHUNK_SIZE)) {
      const supplierRows = await fetchAll<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "supplier_prices", {
...
async function fetchPortalSupplierPriceFallbackRows(
  supabaseUrl: string,
  serviceRoleKey: string,
  organizationId: string,
  brandId: string,
  ...
  priceListType: PortalPriceListRow["price_list_type"],
) {
  const rows = await fetchAllSoft<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "supplier_prices", {
    select: "product_code,normalized_code,description,oem_no,buy_price,valid_from,notes",
    organization_id: `eq.${organizationId}`,
    brand_id: `eq.${brandId}`,
    is_active: "eq.true",
    buy_price: "not.is.null",
  ...
  const bestByCode = new Map<string, {
    product_code: string;
    description: string;
    oem_no: string;
    sell_price: number;
  ...
  for (const row of rows) {
    ...
    const current = bestByCode.get(normalizedCode);
    const next = {
      product_code: String(row.product_code || normalizedCode),
      description: String(row.description || ""),
      oem_no: String(row.oem_no || ""),
      sell_price: roundMoney(buyPrice * (1 + marginPercent / 100)),
      price_date: row.valid_from == null ? null : String(row.valid_from),
      notes: String(row.notes || "").trim(),
    };
    if (!current || next.sell_price < current.sell_price) {
      bestByCode.set(normalizedCode, next);
    }
  }
  return [...bestByCode.values()]
    .sort((left, right) => left.product_code.localeCompare(right.product_code, "en", { numeric: true }))
    .map((row) => ({
      product_code: row.product_code,
      brand: brandName,
      description: row.description,
      price_list_type: priceListType,
      sales_price: row.sell_price,
      price_date: row.price_date,
      lifecycle_status: "active" as const,
      lifecycle_note: row.notes || null,
    }));
}

async function fetchPortalCPriceFallbackRows(
  supabaseUrl: string,
  serviceRoleKey: string,
  organizationId: string,
  brandId: string,
  ...
  if (!cPriceListIds.length) return [];
  const rows = await fetchAllSoft<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "customer_price_list_items", {
    select: "price_list_id,product_code,normalized_code,sell_price,updated_at",
    organization_id: `eq.${organizationId}`,
    price_list_id: `in.(${cPriceListIds.join(",")})`,
    brand_id: `eq.${brandId}`,
    order: "normalized_code.asc",
  ...
  const listPriority = new Map<string, number>();
  cPriceListIds.forEach((id, index) => listPriority.set(id, index));
  const bestByCode = new Map<string, {
    product_code: string;
    sell_price: number;
    price_date: string | null;
  ...
  for (const row of rows) {
    ...
    if (!normalizedCode || sellPrice == null || !Number.isFinite(sellPrice)) continue;
    const priority = listPriority.get(String(row.price_list_id || "")) ?? Number.MAX_SAFE_INTEGER;
    const current = bestByCode.get(normalizedCode);
    const currentPriority = resultPriority.get(normalizedCode);
    if (currentPriority != null && currentPriority < priority) continue;
    if (currentPriority != null && currentPriority === priority && current && String(current.price_date || "") >= String(row.updated_at || "").slice(0, 10)) continue;
    bestByCode.set(normalizedCode, {
      product_code: String(row.product_code || normalizedCode),
      sell_price: sellPrice,
      price_date: row.updated_at == null ? null : String(row.updated_at).slice(0, 10),
    });
    resultPriority.set(normalizedCode, priority);
  }
  return [...bestByCode.values()]
    .sort((left, right) => left.product_code.localeCompare(right.product_code, "en", { numeric: true }))
    .map((row) => ({
...
export async function buildPortalPriceListRows(
  ...
  const brandMap = await resolveBrandMap(supabaseUrl, serviceRoleKey, invite.organization_id);
  const allowedBrandIds = portalAllowedBrandIds(invite);
  const requestedBrand = String(brand || "").trim();
  const cacheKeyBase = [
    invite.organization_id,
    context.customer.id,
  ...
  const selectedBrands = requestedBrand
    ? (() => {
        const brandIds = resolveBrandIdsFromMap(brandMap, requestedBrand);
        const entries = brandIds
          .map((brandId) => ({
            brandId,
            brandName: brandMap.byId.get(brandId) || "",
          }))
          .filter((entry) => entry.brandId && entry.brandName);
        if (!entries.length) {
          throw new Error("Brand not found for portal price list");
        }
        if (allowedBrandIds.length) {
          const scoped = entries.filter((entry) => allowedBrandIds.includes(entry.brandId));
          if (!scoped.length) {
            throw new Error("Brand is outside this portal scope.");
          }
          return scoped;
    ...
    : (allowedBrandIds.length ? allowedBrandIds : [...brandMap.byId.keys()])
        .map((brandId) => ({
          brandId,
          brandName: brandMap.byId.get(brandId) || "",
        }))
  ...
  if (!selectedBrands.length) {
    throw new Error("No brands available for this portal price list.");
  }

  const cacheKey = [...cacheKeyBase, selectedBrands.map((entry) => entry.brandId).join(",") || "ALL"].join("::");
  const cached = portalPriceListCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) {
    return cached.value;
  }

  async function buildRowsForBrand(brandId: string, brandName: string) {
    const brandCacheKey = [...cacheKeyBase, brandId].join("::");
    const cachedBrand = portalPriceListCache.get(brandCacheKey);
    if (cachedBrand && cachedBrand.expiresAt > Date.now()) {
      return cachedBrand.value.rows;
    }

    let catalogRows: Awaited<ReturnType<typeof fetchPortalCatalogBrandRows>> = [];
    try {
      catalogRows = await fetchPortalCatalogBrandRows(supabaseUrl, serviceRoleKey, invite.organization_id, brandId, brandName);
    } catch (error) {
      console.warn(
        JSON.stringify({
          scope: "portal-price-list",
          stage: "catalog-brand-fetch-soft-fail",
          inviteId: invite.id,
          organizationId: invite.organization_id,
          brandId,
          brandName,
    ...
    const salesPriceByCode = new Map<string, number>();
    const priceDateByCode = new Map<string, string | null>();
    const priceTypeByCode = new Map<string, PortalPriceListRow["price_list_type"]>();
    let fallbackRows: PortalPriceListRow[] = [];
    if (!catalogRows.length && context.customerType === "C") {
      fallbackRows = await fetchPortalCPriceFallbackRows(
        supabaseUrl,
        serviceRoleKey,
        invite.organization_id,
        brandId,
      ...
      const marginPercent = getSupplierFallbackCustomerType(context.customerType) === "B" ? context.effectiveMarginB : context.effectiveMarginA;
      fallbackRows = await fetchPortalSupplierPriceFallbackRows(
        supabaseUrl,
        serviceRoleKey,
        invite.organization_id,
        brandId,
        brandName,
        marginPercent,
        getDisplayPriceListType(context.customerType),
      );
    }
    ...
    if (!catalogRows.length) {
      ...
      portalPriceListCache.set(brandCacheKey, {
        value: emptyBrandResult,
        expiresAt: Date.now() + PORTAL_PRICE_LIST_CACHE_TTL_MS,
      });
      return emptyBrandResult.rows;
    }
    ...
    if (context.customerType === "C") {
      try {
        ...
        for (const [key, value] of cPriceEntryMap.entries()) {
          ...
          priceTypeByCode.set(normalizedCode, getDisplayPriceListType(context.customerType));
        }
      } catch (error) {
        console.warn(
          JSON.stringify({
      ...
      try {
        ...
        console.warn(
          JSON.stringify({
            scope: "portal-price-list",
            stage: "supplier-lookup-soft-fail",
            inviteId: invite.id,
            organizationId: invite.organization_id,
            brandId,
            brandName,
    ...
    let brandResultRows = catalogRows.map((row) => ({
      ...
      price_list_type:
        context.customerType === "C"
          ? getDisplayPriceListType(context.customerType)
          : priceTypeByCode.get(row.normalized_code) ?? getDisplayPriceListType(context.customerType),
    ...
    if (!brandResultRows.some((row) => row.sales_price != null)) {
      const fallback = context.customerType === "C"
        ? await fetchPortalCPriceFallbackRows(
            supabaseUrl,
            serviceRoleKey,
            invite.organization_id,
            brandId,
        ...
        : await fetchPortalSupplierPriceFallbackRows(
            supabaseUrl,
            serviceRoleKey,
            invite.organization_id,
            brandId,
            brandName,
            getSupplierFallbackCustomerType(context.customerType) === "B" ? context.effectiveMarginB : context.effectiveMarginA,
            getDisplayPriceListType(context.customerType),
          );
      if (fallback.length) {
        brandResultRows = fallback;
      }
    }
    ...
    return brandResultRows;
  }

  const rows: PortalPriceListRow[] = [];
  for (const entry of selectedBrands) {
    try {
      rows.push(...(await buildRowsForBrand(entry.brandId, entry.brandName)));
    } catch (error) {
      console.warn(
        JSON.stringify({
          scope: "portal-price-list",
          stage: "build-price-list-brand-soft-fail",
          inviteId: invite.id,
          organizationId: invite.organization_id,
  ...
  rows.sort((left, right) => {
    const brandCompare = left.brand.localeCompare(right.brand, "en");
...
async function resolvePortalCatalogSupplierData(
  ...
  const [catalogExact, catalogOem, supplierExact, supplierOem] = await Promise.all([
    fetchFirst<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "catalog_products", {
      select: "product_code,description,oem_no,hs_code,origin,market_segment,weight_kg,image_url,brand_id,normalized_code,normalized_oem,lifecycle_status,lifecycle_note",
  ...
  for (const item of supplierMatchesRaw) {
    ...
    const supplier = (item.suppliers as { name?: unknown } | null | undefined) || null;
...
async function fetchSupplierOptions(
  ...
  const supplierCustomerType = getSupplierFallbackCustomerType(context.customerType);
  let options: Array<Record<string, unknown>> = [];
  try {
    options = (await callRpc<Array<Record<string, unknown>>>(supabaseUrl, serviceRoleKey, "cloud_quote_supplier_options", {
    ...
    console.warn(
      JSON.stringify({
        scope: "portal-order",
        stage: "supplier-options-soft-fail",
        brand: row.brand,
        code: row.code,
        error: String(error instanceof Error ? error.message : error || ""),
      }),
...
function portalBrandCodeKey(brandId: string, normalizedCode: string) {
  return `${String(brandId || "").trim()}::${String(normalizedCode || "").trim()}`;
}

async function resolvePreparedLinesBatch(
  supabaseUrl: string,
  serviceRoleKey: string,
  context: CustomerPricingContext,
  rows: PortalOrderInputRow[],
) {
  const filteredRows = rows.filter((row) => String(row.code || "").trim() && String(row.brand || "").trim());
  if (!filteredRows.length) return [] as PreparedPortalLine[];

  const brandMap = await resolveBrandMap(supabaseUrl, serviceRoleKey, context.organizationId);
  const preparedRows = filteredRows.map((row) => {
    const normalizedBrand = String(row.brand || "").trim().toLowerCase();
    const brandId = brandMap.byName.get(normalizedBrand) || "";
    const canonicalBrand = brandMap.byId.get(brandId) || String(row.brand || "").trim();
    const normalizedRequestedCode = normalizePartCode(String(row.code || ""));
    return {
      row,
      brandId,
      canonicalBrand,
  ...
  const requestedCodes = [...new Set(preparedRows.map((row) => row.normalizedRequestedCode).filter(Boolean))];
  if (!brandIds.length || !requestedCodes.length) {
    return preparedRows.map(({ row, canonicalBrand }) => ({
  ...
  >();
  for (const codeChunk of chunkValues(requestedCodes, PORTAL_PREPARE_QUERY_CHUNK_SIZE)) {
    const referenceRows = await fetchAllSoft<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "item_code_references", {
      select: "brand_id,old_code,new_code,reason,normalized_old_code",
      organization_id: `eq.${context.organizationId}`,
      brand_id: `in.(${brandIds.join(",")})`,
      is_active: "eq.true",
      normalized_old_code: `in.(${codeChunk.join(",")})`,
      limit: String(Math.max(400, codeChunk.length * Math.max(brandIds.length, 1) * 2)),
    });
    for (const reference of referenceRows) {
      const brandId = String(reference.brand_id || "");
      const normalizedOldCode = String(reference.normalized_old_code || normalizePartCode(String(reference.old_code || "")));
      const key = portalBrandCodeKey(brandId, normalizedOldCode);
      if (!brandId || !normalizedOldCode || referenceByKey.has(key)) continue;
      referenceByKey.set(key, {
        old_code: String(reference.old_code || ""),
        new_code: String(reference.new_code || ""),
        reason: String(reference.reason || "").trim() || null,
      });
    }
  }

  const resolvedRows = preparedRows.map((entry) => {
    const referenceMatch = referenceByKey.get(portalBrandCodeKey(entry.brandId, entry.normalizedRequestedCode)) || null;
    const targetCode = referenceMatch?.new_code || entry.row.code;
    return {
      ...entry,
      referenceMatch,
      targetCode,
      normalizedTargetCode: normalizePartCode(String(targetCode || "")),
    };
  });

  const targetCodes = [...new Set(resolvedRows.map((row) => row.normalizedTargetCode).filter(Boolean))];
  const catalogExact = new Map<string, Record<string, unknown>>();
  const catalogOem = new Map<string, Record<string, unknown>>();
  const supplierExact = new Map<string, Array<Record<string, unknown>>>();
  const supplierOem = new Map<string, Array<Record<string, unknown>>>();

  for (const codeChunk of chunkValues(targetCodes, PORTAL_PREPARE_QUERY_CHUNK_SIZE)) {
    const [catalogExactRows, catalogOemRows, supplierExactRows, supplierOemRows] = await Promise.all([
      fetchAllSoft<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "catalog_products", {
        select: "product_code,description,oem_no,hs_code,origin,market_segment,weight_kg,image_url,brand_id,normalized_code,normalized_oem,lifecycle_status,lifecycle_note",
        organization_id: `eq.${context.organizationId}`,
        brand_id: `in.(${brandIds.join(",")})`,
        normalized_code: `in.(${codeChunk.join(",")})`,
        limit: String(Math.max(400, codeChunk.length * Math.max(brandIds.length, 1) * 4)),
      }),
      fetchAllSoft<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "catalog_products", {
        select: "product_code,description,oem_no,hs_code,origin,market_segment,weight_kg,image_url,brand_id,normalized_code,normalized_oem,lifecycle_status,lifecycle_note",
        organization_id: `eq.${context.organizationId}`,
        brand_id: `in.(${brandIds.join(",")})`,
        normalized_oem: `in.(${codeChunk.join(",")})`,
        limit: String(Math.max(400, codeChunk.length * Math.max(brandIds.length, 1) * 4)),
      }),
      fetchAllSoft<Record<string, unknown>>(supabaseUrl, serviceRoleKey, "supplier_prices", {
        select: "supplier_id,brand_id,product_code,normalized_code,normalized_oem,description,oem_no,buy_price,valid_from,notes,suppliers(name)",
        organization_id: `eq.${context.organizationId}`,
        brand_id: `in.(${brandIds.join(",")})`,
        is_active: "eq.true",
    ...
    for (const row of catalogExactRows) {
      const key = portalBrandCodeKey(String(row.brand_id || ""), String(row.normalized_code || normalizePartCode(String(row.product_code || ""))));
      if (!catalogExact.has(key)) catalogExact.set(key, row);
    }
    for (const row of catalogOemRows) {
      const key = portalBrandCodeKey(String(row.brand_id || ""), String(row.normalized_oem || normalizePartCode(String(row.oem_no || ""))));
      if (!catalogOem.has(key)) catalogOem.set(key, row);
    }
    for (const row of supplierExactRows) {
      const key = portalBrandCodeKey(String(row.brand_id || ""), String(row.normalized_code || normalizePartCode(String(row.product_code || ""))));
  ...
  let cPriceEntryMap = new Map<string, { sell_price: number; price_date: string | null }>();
  if (shouldUsePortalCPrice(context)) {
    try {
      cPriceEntryMap = await fetchCPriceEntryMap(
        supabaseUrl,
        serviceRoleKey,
        ...
        resolvedRows.map((entry) => {
          const catalogMatch =
            catalogExact.get(portalBrandCodeKey(entry.brandId, entry.normalizedTargetCode)) ||
            catalogOem.get(portalBrandCodeKey(entry.brandId, entry.normalizedTargetCode)) ||
            null;
          return {
            brand: entry.canonicalBrand,
            product_code: String(catalogMatch?.product_code || entry.targetCode || entry.row.code || ""),
          };
        }),
      );
    } catch (error) {
  ...
  return resolvedRows.map((entry) => {
    const key = portalBrandCodeKey(entry.brandId, entry.normalizedTargetCode);
    ...
    const uniqueSupplierMatches = new Map<string, {
      supplier_id?: string | null;
      supplier_name: string;
      buy_price: number | null;
      price_date: string | null;
    ...
    for (const item of supplierMatchesRaw) {
      ...
      const supplierName = String(supplier?.name || "");
      const buyPrice = item.buy_price == null ? null : Number(item.buy_price);
      if (!supplierName || buyPrice == null || !Number.isFinite(buyPrice)) continue;
      const supplierKey = `${supplierId || ""}::${supplierName}`;
      const current = uniqueSupplierMatches.get(supplierKey);
      if (current && Number(current.buy_price ?? Number.MAX_SAFE_INTEGER) <= buyPrice) continue;
      uniqueSupplierMatches.set(supplierKey, {
        supplier_id: supplierId,
        supplier_name: supplierName,
        buy_price: buyPrice,
        sell_price: computeSellFromBuy(buyPrice, context),
    ...
    const supplierOptions = [...uniqueSupplierMatches.values()].sort(
      (left, right) => Number(left.buy_price ?? Number.MAX_SAFE_INTEGER) - Number(right.buy_price ?? Number.MAX_SAFE_INTEGER),
    );
    const selectedSupplier = supplierOptions[0] || null;
    const resolvedCode = String(catalogMatch?.product_code || entry.targetCode || entry.row.code || "");
    const cPriceEntry =
      cPriceEntryMap.get(`${entry.canonicalBrand.trim().toLowerCase()}::${normalizePartCode(resolvedCode)}`) || null;
    ...
    const codeChanged = Boolean(entry.referenceMatch) || normalizePartCode(resolvedCode) !== entry.normalizedRequestedCode;
    let sellPrice = selectedSupplier?.sell_price ?? null;
    let cSellPrice = cPriceEntry?.sell_price ?? null;
    let priceDate = String(selectedSupplier?.price_date || "");

    if (cPriceEntry) {
      sellPrice = Number(cPriceEntry.sell_price);
      cSellPrice = Number(cPriceEntry.sell_price);
      priceDate = String(cPriceEntry.price_date || priceDate || "");
    ...
    return {
      lineId: makeId("portal-line"),
      requestedCode: entry.row.code,
      resolvedCode,
      brand: entry.canonicalBrand,
      market_segment: normalizeCatalogMarketSegment(catalogMatch?.market_segment || entry.row.market_segment || null),
      description: String(catalogMatch?.description || supplierMatchesRaw[0]?.description || ""),
      qty: entry.row.qty,
      oem_no: sanitizeCatalogOemNumbers(catalogMatch?.oem_no || supplierMatchesRaw[0]?.oem_no),
      ...
      supplier_name: String(selectedSupplier?.supplier_name || ""),
      buy_price: selectedSupplier?.buy_price ?? null,
      sell_price: sellPrice,
      c_sell_price: cSellPrice,
      price_date: priceDate,
      notes: String(selectedSupplier?.notes || ""),
      found: Boolean(catalogMatch || supplierOptions.length || cPriceEntry),
      codeChanged,
      codeChangeWarning: entry.referenceMatch
        ? `Old Code ${entry.referenceMatch.old_code} => New Code ${entry.referenceMatch.new_code}.${entry.referenceMatch.reason ? ` ${entry.referenceMatch.reason}` : ""}`
        : codeChanged
          ? `Old Code ${entry.row.code} => New Code ${resolvedCode}`
          : "",
      ...
      replacement_code: entry.referenceMatch?.new_code || null,
      replacement_old_code: entry.referenceMatch?.old_code || null,
      replacement_reason: entry.referenceMatch?.reason || null,
      replacement_warning: entry.referenceMatch
        ? buildReplacementWarning(entry.referenceMatch.old_code, entry.referenceMatch.new_code || resolvedCode, entry.referenceMatch.reason)
        : null,
      supplierOptions,
      selectedSupplierKey: selectedSupplier ? `${selectedSupplier.supplier_name}-0` : "",
    } satisfies PreparedPortalLine;
  });
}
...
async function resolvePreparedLine(
  ...
  return {
    ...
    resolvedCode,
    brand: row.brand || "",
    market_segment: normalizeCatalogMarketSegment(catalogMatch?.market_segment || row.market_segment || null),
...
export async function preparePortalOrderLines(
  ...
  for (let index = 0; index < mergedRows.length; index += PORTAL_PREPARE_BATCH_SIZE) {
    const chunk = mergedRows.slice(index, index + PORTAL_PREPARE_BATCH_SIZE);
    const resolvedChunk = await resolvePreparedLinesBatch(supabaseUrl, serviceRoleKey, context, chunk);
    prepared.push(...resolvedChunk);
  }
  console.info(
    JSON.stringify({
      scope: "portal-order-prepare",
      stage: "prepared-lines",
      inviteId: invite.id,
      organizationId: invite.organization_id,
